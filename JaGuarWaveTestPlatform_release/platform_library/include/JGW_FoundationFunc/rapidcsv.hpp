...
namespace std
{
    ...
    inline string to_string(size_t _Val)
    {	// convert unsigned long long to string
        char _Buf[2 * _MAX_INT_DIG];
...
//#define std::function<void (const std::string & pStr, T & pVal)> std::function<void (const std::string & pStr, T & pVal)>
  /**
  ...
  public:
     ...
     */
    template<typename T>
    std::vector<T> GetColumn(const size_t pColumnIdx, std::function<void (const std::string & pStr, T & pVal)> pToVal) const
    {
      const ssize_t columnIdx = pColumnIdx + (mLabelParams.mRowNameIdx + 1);
      std::vector<T> column;
      for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow)
     ...
     */
    template<typename T>
    std::vector<T> GetColumn(const std::string& pColumnName, std::function<void (const std::string & pStr, T & pVal)> pToVal) const
    {
      const ssize_t columnIdx = GetColumnIdx(pColumnName);
      if (columnIdx < 0)
      {
     ...
     */
    template<typename T>
    std::vector<T> GetRow(const size_t pRowIdx, std::function<void (const std::string & pStr, T & pVal)> pToVal) const
    {
      const ssize_t rowIdx = pRowIdx + (mLabelParams.mColumnNameIdx + 1);
      std::vector<T> row;
      Converter<T> converter(mConverterParams);
     ...
     */
    template<typename T>
    std::vector<T> GetRow(const std::string& pRowName, std::function<void (const std::string & pStr, T & pVal)> pToVal) const
    {
      ssize_t rowIdx = GetRowIdx(pRowName);
      if (rowIdx < 0)
      {
     ...
     */
    template<typename T>
    T GetCell(const size_t pColumnIdx, const size_t pRowIdx, std::function<void (const std::string & pStr, T & pVal)> pToVal) const
    {
      const ssize_t columnIdx = pColumnIdx + (mLabelParams.mRowNameIdx + 1);
      const ssize_t rowIdx = pRowIdx + (mLabelParams.mColumnNameIdx + 1);
     ...
     */
    template<typename T>
    T GetCell(const std::string& pColumnName, const std::string& pRowName, std::function<void (const std::string & pStr, T & pVal)> pToVal) const
    {
      const ssize_t columnIdx = GetColumnIdx(pColumnName);
      if (columnIdx < 0)
      {
     ...
     */
    template<typename T>
    T GetCell(const std::string& pColumnName, const size_t pRowIdx, std::function<void (const std::string & pStr, T & pVal)> pToVal) const
    {
      const ssize_t columnIdx = GetColumnIdx(pColumnName);
      if (columnIdx < 0)
      {
     ...
     */
    template<typename T>
    T GetCell(const size_t pColumnIdx, const std::string& pRowName, std::function<void (const std::string & pStr, T & pVal)> pToVal) const
    {
      const ssize_t rowIdx = GetRowIdx(pRowName);
      if (rowIdx < 0)
      {
  ...
  private:
    ...
    void ReadCsv(std::istream& pStream)
    {
...
#endif
      {
        ...
        if (length >= 3)
        {
          ...
          #if 0
          std::vector<char> bomU8;
          bomU8.push_back('\xef');
          bomU8.push_back('\xbb');
          bomU8.push_back('\xbf');
          if (bom3b != bomU8)
          #else
          if ('\xef' != bom3b[0] || '\xbb' != bom3b[1] || '\xbf' != bom3b[2])
          #endif
          {
    ...
    void ParseCsv(std::istream& pStream, std::streamsize p_FileLength)
    {
...
#else
        for (size_t index = 0;index < mData[mLabelParams.mColumnNameIdx].size();index++)
        {
            mColumnNames[mData[mLabelParams.mColumnNameIdx][index]] = i++;
        }
#endif
      }
           ...
           (mLabelParams.mColumnNameIdx + 1)))
      {
        ...
        {
            if (static_cast<ssize_t>(mData[index].size()) > mLabelParams.mRowNameIdx)
            {
                mRowNames[mData[index][mLabelParams.mRowNameIdx]] = i++;
            }
        }
#endif
      }
