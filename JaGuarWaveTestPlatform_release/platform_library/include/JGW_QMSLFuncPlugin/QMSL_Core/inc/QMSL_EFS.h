/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Core/inc/QMSL_EFS.h#6 $
 * $DateTime: 2019/07/29 08:44:11 $
 *
...
//! Call back for an EFS directory element
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_EFS2_DIAG_MAKE_GOLDEN_COPY(
     ...
     /**
   EFS2,  80-V1294-11 3.59 EFS2_DIAG_MAKE_GOLDEN_COPY_V2 request/response

   This command packet requests the creation of a backup (golden copy) of the current
   state of the file system. Creation of a backup for the file system is not supported for all
   storage devices (e.g., Flash). DIAG can only be used to create a backup once over the
   life of the device. If a power loss occurs during backup creation, then the golden-copy
   created could be a corrupt one.

    This command queues a golden-copy request and returns immediately and the status of
    this queued golden-copy request is obtained by using the
   EFS2_DIAG_MAKE_GOLDEN_COPY_V2_GET_STATUS command.

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param iSequenceNum = SEQUENCE_NUMBER is used for synchronization/correlation between similar commands; should be a unique number generated by the application to match requests with responses

   \param iGoldenCopyType = How the Golden-copy should be created?

   01 - Whole snapshot image of current EFS
   02 - golden-copy comprising only selected list of files
   03 - golden-copy comprising only OEM_1 selected list of files
   04 - golden-copy comprising only OEM_2 selected list of files 
   05 - Zipped-TAR file of current EFS image or of only selected list of files

    \param sFilePath = Path name (NULL-terminated string)

   \param iGolenCopyStatus = Golden copy status
   ...
   *******************************************************************************/
      ...
      QLIB_API unsigned char QLIB_EFS2_DIAG_MAKE_GOLDEN_COPY_V2 (
      HANDLE hResourceContext,
      unsigned long iSequenceNum,
      unsigned long iGoldenCopyType,
      unsigned char *sFilePath,
      unsigned long *iGolenCopyStatus,
      unsigned long* pERRNO);

     /**
   EFS2,  80-V1294-11 3.60 EFS2_DIAG_MAKE_GOLDEN_COPY_V2_GET_STATUS  request/response

   This command packet requests the status of a queued golden copy creation request.
   This command can be issued any number of times.
   ...
   *******************************************************************************/
      ...
      QLIB_API unsigned char QLIB_EFS2_DIAG_MAKE_GOLDEN_COPY_V2_GET_STATUS(
      HANDLE hResourceContext,
      unsigned long iSequenceNum,
      unsigned char *sFilePath,
      unsigned long *iGoldenCopyStatus,
      unsigned long* pERRNO);
   ...
   *******************************************************************************/
   ...
   Set EFS Subsystem cmd id to select primary or alternative filesystem access :

      \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()
      \param bEFS_SubSystemMode    = 0: Primary filesystem, 1: Alternative filesystem
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_Configure_EFS_SubSystemMode(HANDLE hResourceContext, unsigned char bEFS_SubSystemMode /* 0: Primary, 1: Alternative */);
   /******************************************************************************/
   /**


   Get current EFS Subsystem cmd id to select primary or alternative filesystem access :
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_Get_Current_EFS_SubSystemMode(HANDLE hResourceContext, unsigned char * bCurrentEFS_SubSystemMode /* 0: Primary, 1: Alternative */);
