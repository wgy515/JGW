/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_WWAN/inc/QMSL_FTM_GSM_RF.h#24 $
 * $DateTime: 2021/02/11 20:23:12 $
 *
...
/******************************************************************************/
/**
Factory Test Mode GSM Commands 3.22 Rx gain range cal for target Chiltepin
...
\note This function can be used in QMSL FTM Sequencer operation
*******************************************************************************/
QLIB_API unsigned char QLIB_FTM_RX_GAIN_RANGE_CAL_V2(
   HANDLE hResourceContext,
   unsigned char iRxRangeBitMask,
   unsigned char iNumAverages,
   unsigned char bToggleRx,
   unsigned long aiRSSI_Result[8],
   signed long aiDCVal_I_Result[8],
   signed long aiDCVal_Q_Result[8],
   unsigned char benDCCal
);
...
This command sets GSM IRAT INPUT DATA
based on the packet information from diag.
we pack the information below into the diag packet:
1. Sub ID for which the FTM IRAT will occur
2. Expected IoR in dB10

\param subID = Subscription ID on which FTM IRAT will occur
0 - SUB 0
1 - SUB 1

\param exp_level_db10 = expected level in dB10 units

\return true if successful, false if fail.

\warning This API is applicable from HI.3.0 modem package

*******************************************************************************/
QLIB_API unsigned char QLIB_FTM_GSM_SET_IRAT_INPUT_DATA(HANDLE hResourceContext, unsigned char subID, short exp_level_db10);

/******************************************************************************/
/**
Factory Test Mode GSM RF Test Commands CL93-V5370-1

This command gets GSM IRAT OUTPUT DATA
based on the packet information from diag.
we pack the information below into the diag packet:
1. Sub ID for which the FTM IRAT will occur
...
\param measured_rssi = measured rssi in db10 (response)
...
*******************************************************************************/
QLIB_API unsigned char QLIB_FTM_GSM_GET_IRAT_OUTPUT_DATA(HANDLE hResourceContext, unsigned char subID, double* measured_rssi);
...
/******************************************************************************/
/**
QLIB_FTM_GSM_SET_RFM_PATH_V2

Uses FTM_GSM_SET_RFM_PATH (FTM API) command for setting up PRx, DRx and Tx RFM Paths(SigPath + RFM Device + Antenna Path + PLL ID) to use in FTM mode.

\param hResourceContext   = Resource context that was returned from the call to QLIB_ConnectServer()
\param iNumBands          = Number of bands
\param iGsmBandRFMPathMap = Struct of ftm_gsm_set_rfm_path_type_v2

\return true if successful, false if fail.

\warning None.
*******************************************************************************/
QLIB_API unsigned char QLIB_FTM_GSM_SET_RFM_PATH_V2( HANDLE hResourceContext, unsigned char iNumBands, void* iGsmBandRFMPathMap);
