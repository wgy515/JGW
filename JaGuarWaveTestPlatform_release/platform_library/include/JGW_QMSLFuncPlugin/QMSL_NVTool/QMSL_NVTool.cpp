/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Projects/QMSL_NVTool/QMSL_NVTool.cpp#39 $
 * $DateTime: 2020/03/31 16:13:48 $
 *
 *
 ******************************************************************************
 *
...
const int NVTOOL_FAILURE_IN_QMSL_API = 3;  // Should this maybe be -3
const int NVTOOL_PHONE_CONNECTION_FAILED = -4;
...
bool Ph2src_l();  //read NV values from mobile and write to NV source xml  QRCT3 Functionality\n"
...
bool LoadNVsFromMobile_legacy();   // Load NVs from mobile QRCT3 functionality
...
unsigned char bQLIB_LIB_Mode = QLIB_LIB_MODE_QPHONEMS;
// Use MSM target
unsigned char bMSMTarget = 1;
...
int _tmain(int argc, _TCHAR* argv[])
{
   ...
   if(CheckArguments() == true)
   {
      ...
      {
         DbgPrint("Failed to connect phone. Make sure phone is powered on and connected to correct COM port!\n");
         rc = NVTOOL_PHONE_CONNECTION_FAILED;
      }
      DisconnectServer();
   }
   else
...
void ParseArguments()
{
   ...
   for(_i = 2; _i < gArgumentList.size(); _i++)
   {
      ...
      else if ((_position = argument.find("-qphonems")) == 0)
      {
          bQLIB_LIB_Mode = QLIB_LIB_MODE_QPHONEMS;
      }
      else if ((_position = argument.find("-quts")) == 0)
      {
          bQLIB_LIB_Mode = QLIB_LIB_MODE_QUTS;
      }
      else if ((_position = argument.find("-apq")) == 0)
      {
         bMSMTarget = 0;
      }
      else if ((_position = argument.find("-msm")) == 0) // by default MSMTarget = 1
      {
         bMSMTarget = 1;
      }
...
void PrintUsage()
{
   ...
   printf("\t-ph2tsrc   read NV values from mobile and write to NV source xml. This has the QRCT 3 functionality\n");
   ...
   printf("\t -qphonems Use QPHONEMS when communicating with mobile\n");
   printf("\t -quts Use QUTS when communicating with mobile\n");
...
bool LoadNVsFromMobile_legacy()   // Load NVs from mobile (QRCT3 compatibility)
{
   bool bResult = true;
   int _iResultCode = 0;
   int _iNumNVValues = 0;
   ...
   ::QLIB_NV_LoadNVsFromMobile_legacy(gQMSL, &_iNumNVValues, &_iResultCode);
...
bool ConnectServer()
{
   QLIB_SetLibraryMode(bQLIB_LIB_Mode);

   // Set target Type MSM or APQ
   QLIB_SetTargetType( (1 == bMSMTarget)? QLIB_TARGET_TYPE_MSM_MDM : QLIB_TARGET_TYPE_APQ);
   ...
   if( (gCmdAction == SRC2PH)  ||
       ...
       (gCmdAction == QPST2QCN)  )
   {
	  unsigned long iWaitTime = 10000;
	  gQMSL = ::QLIB_ConnectServerWithWait(gComPort, iWaitTime);
