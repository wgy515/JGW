...
namespace JGW
{
	std::string gstrHeader;

	CCJGW_CurlHttp::CCJGW_CurlHttp(const std::string& strSourceAddress)
	{
		memset(mszSourceAddress,0x00,128);
		strcpy_s(mszSourceAddress,128,strSourceAddress.c_str());
	}
	...
	void CCJGW_CurlHttp::SetSourceAddress(const std::string& strSourceAddress)
	{
		memset(mszSourceAddress,0x00,128);
		strcpy_s(mszSourceAddress,128,strSourceAddress.c_str());
	}
	...
	size_t header_callback(char *buffer, size_t size, size_t nitems, void *userdata) {
		...
		if (strstr(buffer, "Content-Length:")) {
			sscanf(buffer, "Content-Length: %lf", (double*)userdata);
		}
		return size * nitems;
	}

#if 1
	bool CCJGW_CurlHttp::SendCurlHttpA(const std::string& strUrl,bool bSaveCookieFile,bool bUseCookieFile,const std::string& strCookiePath,std::string& strResponceUtf8,std::string& strErrorMsgUtf8)
	{
		CURL *curl = NULL;
		...
		std::string strUrlUtf8 = JGW_LocalToUTF8(strUrl);
		std::string strCookiePathUtf8 = JGW_LocalToUTF8(strCookiePath);
		gstrHeader = "";
		/* In windows, this will init the winsock stuff */
		// 虽然 libcurl 本身线程安全，但 curl_global_init 不是线程安全的。需在主线程中调用一次，避免多线程重复初始化
		...
		{
			// 设置请求URL（示例）
			curl_easy_setopt(curl, CURLOPT_URL,strUrlUtf8.c_str());
#if 1
			// -b 参数等效实现（设置Cookie）
			if (bUseCookieFile)
			{
				curl_easy_setopt(curl, CURLOPT_COOKIEFILE, strCookiePathUtf8.c_str());
			}
			if (bSaveCookieFile)
			{
				// -c 参数等效实现（保存Cookie到文件）
				curl_easy_setopt(curl, CURLOPT_COOKIEJAR, strCookiePathUtf8.c_str());
			}
#endif

			//! url绑定特定网口作为出口
			//curl_easy_setopt(curl, CURLOPT_INTERFACE, mszSourceAddress);
			if (strlen(mszSourceAddress) >= 8) 
			{
				curl_easy_setopt(curl, CURLOPT_INTERFACE, mszSourceAddress);
			}

			// 使用时：
			double content_length = 0.00;
			...
			if(res != CURLE_OK)
			{
				JGW_FormatString(strErrorMsgUtf8,"curl_easy_perform() failed: %s\n",curl_easy_strerror(res));
			}
			curl_slist_free_all(headers); /* free the list again */
			strResponceUtf8 = out.str();//返回请求值 
			/* always cleanup */
...
#endif
	bool CCJGW_CurlHttp::SendCurlHttp(const std::wstring& strUrl,bool bSaveCookieFile,bool bUseCookieFile,const std::wstring& strCookiePath,std::wstring& strResponce,std::wstring& strErrorMsg)
	{
		CURL *curl = NULL;
		CURLcode res = CURLE_OK;
		std::stringstream out;
		...
		if(curl) 
		{
			...
			if (bUseCookieFile)
			{
				curl_easy_setopt(curl, CURLOPT_COOKIEFILE, JGW_W2A(strCookiePath,CP_UTF8).c_str());
			}
			if (bSaveCookieFile)
			{
				// -c 参数等效实现（保存Cookie到文件）
				curl_easy_setopt(curl, CURLOPT_COOKIEJAR, JGW_W2A(strCookiePath,CP_UTF8).c_str());
			}
#endif
