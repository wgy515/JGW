...
namespace JGW
{
    ...
    void CCJGW_CSerialComPort::EraseCliString(std::string& strRead)
    {
     ...
     bool CCJGW_CSerialComPort::OpenSerialComPort(std::wstring strComName,bool isSetControlCommState,int nbaud_rate /* = CBR_9600 */,DWORD dwFlagsAndAttributes /* = FILE_FLAG_OVERLAPPED */)
     {
         CloseSerialComPort();
...
//     }
    ...
    int CCJGW_CSerialComPort::ReadAsyncCommand(char* strRead,size_t len,long millis /* = 0 */)
    {
        ...
        while (tm.elapsed() <= fMillis)
        {
            ...
            if (bReader)
            {
                ...
                {
                    Log4WI_F(L"read GetOverlappedResultPortEx ERROR:%s" ,JGW_GetSystemErrorString(GetLastError()).c_str());
                    return -1;
                }
                //! dwBytesRead = 0;
            }
    ...
    int CCJGW_CSerialComPort::WriteAsyncCommand(const char* szATCommand,size_t len,long millis /* = 0 */)
    {
        ...
        if (!bResult && !bWrite)
        {
            Log4WE(L"WriteAsyncATModem Fail");
            return -1;
        }
        ...
        if (bWrite)
        {
            ...
            {
                Log4WE_F(L"WriteAsyncATModem GetOverlappedResultPortEx ERROR:%s" ,JGW_GetSystemErrorString(GetLastError()).c_str());
                return -1;
            }  
        }
    ...
    bool CCJGW_CSerialComPort::CheckIsAsyncRWCoammnd(const char* szATCommand)
    {
        ...
        {
            Log4WE(L"The read/write cache cannot be empty or the current IO is not in asynchronous mode");
            return false;
        }
        return true;
    }
    ...
    void CCJGW_CSerialComPort::GetATCommandString(const char* szATCommand,size_t &len)
    {
        ...
        mTxBuf[len] = L'\0';
        Log4WI_F(L"SerialComPort Send : %s",JGW_A2W_A((char*)mTxBuf).c_str());
        mTxBuf[len] = endChar;
        mTxBuf[++len] = L'\0';
    }
    ...
    bool CCJGW_CSerialComPort::SetControlCommState(DCB dcb)
    {
        return mSerialPort.SetControlCommState(&dcb);
    }

    void CCJGW_CSerialComPort::GetControlCommState(DCB& dcb)
    {
