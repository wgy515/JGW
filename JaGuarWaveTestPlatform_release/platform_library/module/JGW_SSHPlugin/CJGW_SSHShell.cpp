...
namespace JGW
{
	...
	static int auth_pw = 0;
    void CCJGW_SSHShell::DisConnectSSHShell()
    {
        if (NULL != mpSession)
        {
            libssh2_session_set_timeout(mpSession, 1000);
            libssh2_session_set_blocking(mpSession, 0); // 设置为阻塞模式
        }
        
        if (mpChannel)
        {
            libssh2_channel_close(mpChannel);
        ...
        if (mpSession)
        {
            ...
            //! libssh2_session_set_blocking(session, 1); // 设置为阻塞模式
