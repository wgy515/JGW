...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
  ...
  <xsl:variable name="dashLogged">30</xsl:variable>
    ...
    <xsl:copy>
      ...
      <xsl:for-each select="Test[generate-id() = generate-id(key('IndexDistinct', @I)[1])]">
        ...
        <Test>
          ...
          <xsl:if test="$alltests[@I=$currentInd][@invalid_results='true']">
            <!-- if this test returns any elements, the overall status will be invalid -->
            <xsl:attribute name="invalid_results">true</xsl:attribute>
          </xsl:if>
          ...
          <!-- assign 'fail' if any tests have a fail status, otherwise mark pass -->
          <xsl:if test="$alltests[@I=$currentInd]/Time">
            <Time>
              <Duration>
                <HH>
      ...
      <tr>
        <xsl:if test="not(count(Inputs/DI)=0)">
          <xsl:for-each select="Inputs/DI">
            <xsl:choose>
              <xsl:when test="count(@lp)=0 or (@lp &lt; $priorityCutOff)">
                <td align="center">
                  <xsl:value-of select="V"/>
                </td>
              </xsl:when>
              <xsl:when test="@lp=$dashLogged">
                <td align="center">-</td>
              </xsl:when>
            </xsl:choose>
          </xsl:for-each>
        ...
        <xsl:for-each select="Outputs/Result">
          <xsl:choose>
            <xsl:when test="count(@lp)=0 or (@lp &lt; $priorityCutOff)">
              <xsl:choose>
                <xsl:when test="@invalid='true'">
                  <td bgcolor="#EEDD00" align="center">
            ...
            <tr>
              ...
              <xsl:choose>
                <xsl:when test="$pf='PASS'">
                  <xsl:choose>
                    <xsl:when test="@invalid_results='true'">
                      <td bgcolor="#EEDD00">PASS</td>
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <xsl:when test="PassFail/DI/V='PASS'">
                  <xsl:choose>
                    <xsl:when test="@invalid_results='true'">
                      <big>
                        <b>
                          <span style='color:#EEDD00'>PASS</span>
                        </b>
                      </big>
                    </xsl:when>
                    <xsl:otherwise>
