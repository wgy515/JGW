#include "StdAfx.h"
#include "JGW_Cef3HandleBindingsRendererDelegate.h"
#include "performance_test_setup.h"
#include "JGW_Cef3HelpApp.h"

using namespace client::performance_test;
...
namespace JGW
{
                    ...
                    if (arguments.size() == 1 && arguments[0]->IsString()) {
                        ...
                        for (int i = 0; i < kPerfTestsCount; ++i) {
                            if (test == kPerfTests[i].name) {
                                ...
                                retval = CefV8Value::CreateInt((int32)delta);
                                found = true;
                                break;
                            }
                        }
    ...
    JGW_Cef3HandleBindingsRendererDelegate::JGW_Cef3HandleBindingsRendererDelegate(void)
    {
    }
    ...
    void JGW_Cef3HandleBindingsRendererDelegate::OnContextCreated(CefRefPtr<JGW_Cef3HelpApp> app, CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefRefPtr<CefV8Context> context)
    {
