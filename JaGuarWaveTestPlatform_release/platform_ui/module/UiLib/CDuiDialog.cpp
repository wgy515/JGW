...
namespace UiLib
{
	CDuiDialog::CDuiDialog(void):m_strSkinFilePath(_T(""))
	...
	LRESULT CDuiDialog::ResponseDefaultKeyEvent(WPARAM wParam)
	{
		if (wParam == VK_RETURN)
		{
			return FALSE;
	...
	HWND CDuiDialog::CreateDuiDialogWindow(LPCTSTR strSkinFolder ,LPCTSTR strSkinFile,DWORD dwStyle /* = UI_WNDSTYLE_DIALOG */,DWORD dwExStyle /* = WS_EX_STATICEDGE | WS_EX_APPWINDOW */,LPCTSTR pstrWindowName /* = _T */)
	{
	...
	HWND CDuiDialog::CreateDuiDialogWindow(LPCTSTR strSkinFolder ,LPCTSTR strSkinFile,LPCTSTR pstrWindowName /* = _T("CHDuilibWnd") */,DWORD dwStyle /* = UI_WNDSTYLE_DIALOG */,DWORD dwExStyle /* = WS_EX_STATICEDGE | WS_EX_APPWINDOW */)
	{
		return CreateDuiDialogWindow(strSkinFolder,strSkinFile,dwStyle,dwExStyle,pstrWindowName);
	}

	HWND CDuiDialog::CreateDuiDialogWindow(HWND hwndParent,LPCTSTR strSkinFolder ,LPCTSTR strSkinFile,DWORD dwStyle /* = UI_WNDSTYLE_DIALOG */,DWORD dwExStyle /* = WS_EX_STATICEDGE | WS_EX_APPWINDOW */,LPCTSTR pstrWindowName /* = _T("CHDuilibWnd") */)
	{
		SetSkinFolder(strSkinFolder);
		SetSkinFileName(strSkinFile);
		return CreateDuiWindow(hwndParent, dwStyle,dwExStyle ,pstrWindowName /*, 0, 0, 600, 320*/);
	}

	HWND CDuiDialog::CreateDuiWindow(HWND hwndParent,DWORD dwStyle /* =0 */, DWORD dwExStyle /* =0 */, LPCTSTR pstrWindowName)
	{
		m_bIsCreateDuiWnd = true;
		return Create(hwndParent,pstrWindowName,dwStyle,dwExStyle,0,0,0,0,NULL);
	}

	void CDuiDialog::SetSkinFileName(LPCTSTR strSkinFilePath)
	{
		m_strSkinFilePath = strSkinFilePath;
	}

	LPCTSTR CDuiDialog::GetSkinFileName()
	{
		return m_strSkinFilePath.GetData();
	}

	void CDuiDialog::SetSkinFolder(LPCTSTR strSkinFolder)
	{
		m_strSkinFolder = strSkinFolder;
	}

	void CDuiDialog::SetResourceType(UILIB_RESOURCETYPE eResourceType,DWORD dwResourceID /* = 0 */,LPCTSTR strResourceName /* = _T("ZIPRES") */)
	{
	...
	LPCTSTR CDuiDialog::GetSuperClassName()
	{
		return NULL;
	}
	...
	LRESULT CDuiDialog::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM , bool& )
	{
		if (uMsg == WM_KEYDOWN)
		{
			switch (wParam)
	...
	LRESULT CDuiDialog::OnMouseHover(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		bHandled = FALSE;
		return 0;
	}
	...
	LRESULT CDuiDialog::OnChar(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		bHandled = FALSE;
		return 0;
	}
	...
	LRESULT CDuiDialog::HandleCustomMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		const AFX_DUI_MSGMAP* lpWndMessageMap = GetMessageMap();
		const S_WND_MESSAGE* lpWndMessage = NULL;
		int i = 0;
		...
		while ( lpWndMessageMap )
		{
			...
			while ( 0 != lpWndMessage[i].mssageID )
			{
				if (0 == lpWndMessage[i].mssageID || NULL == lpWndMessage[i].myFunc)
				{
					continue;
				}

				if (uMsg == lpWndMessage[i].mssageID)
				{
					(this->*(lpWndMessage[i].myFunc))(wParam,lParam);
					bHandled = TRUE;
					break;
				}
				i ++;
	...
	bool CDuiDialog::LoadLayoutFile()
	{
	...
	const AFX_DUI_MSGMAP* CDuiDialog::GetThisMessageMap()
	{
		return NULL;
	}

	const  AFX_DUI_MSGMAP* CDuiDialog::GetMessageMap() const
	{
		return NULL;
	}
}