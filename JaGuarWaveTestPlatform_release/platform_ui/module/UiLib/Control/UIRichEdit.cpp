...
#else
	size_t nLength = _tcslen(pstrText);
	bool bHandle = true;
	this->SetFocus();

	for ( size_t i = 0;i < nLength;i ++)
	{
		MessageHandler(WM_CHAR, WPARAM(pstrText[i]),NULL,bHandle);
	}
#endif
}
...
#endif
		if(wParam == 'C' && GetAsyncKeyState(VK_CONTROL) < 0) {
			if(IsFocused()) {
				CopyToClipboard();
				return 0;
			}
...
void GL_CopyToClipboard(CRichEditUI* pRichEdit)
{
	if(pRichEdit && !pRichEdit->GetSelText().IsEmpty()) {
		if(OpenClipboard(NULL)) {
			EmptyClipboard();
			HGLOBAL hMem = GlobalAlloc(GMEM_MOVEABLE, 
				(pRichEdit->GetSelText().GetLength() + 1) * sizeof(TCHAR));
			if(hMem) {
				LPTSTR lptstrCopy = (LPTSTR)GlobalLock(hMem);
				_tcscpy_s(lptstrCopy, pRichEdit->GetSelText().GetLength() + 1, 
					pRichEdit->GetSelText());
				GlobalUnlock(hMem);
...
void CRichEditUI::CopyToClipboard() {
#if 0
	CHARRANGE cr = {0};
	GetSel(cr);
	if(cr.cpMin != cr.cpMax) {
		SendMessage(EM_COPY, 0, 0);
	}
#else
	GL_CopyToClipboard(this);
#endif
