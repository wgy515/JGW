...
namespace UiLib
{
	...
	void CEditUI::SendCEditWndMsg( UINT msg,WPARAM wParam,LPARAM pParam )
	{
		::SendMessage(*m_pWindow, msg, wParam,pParam );
	}
...
MatchFailed:
	...
	void CEditUI::SetRegularWarn(LPCTSTR pRegularWarnStr)
	{
		m_RegluarWarnStr = pRegularWarnStr;
	}

	LPCTSTR CEditUI::GetRegularWarn()
	{
		return m_RegluarWarnStr;
	}
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("regularwarn")) == 0) SetRegularWarn(pstrValue);
