...
namespace UiLib
{
	...
	/*******************************************/
	ZQCChartXYAxis::ZQCChartXYAxis(std::vector<std::wstring>& xChartLabels,std::vector<std::wstring>& yChartLabels)
		:m_pChartView(NULL)
		,m_rXChartLabels(xChartLabels)
		,m_rYChartLabels(yChartLabels)
		,m_nLineWidth(1)
		,m_uXYAxisLineSize(2),m_uXYAxisLineColor(RGB(32,32,32)),m_uLineColor(RGB(8,137,241)),m_bVisibleBaseLine(true),m_uBaseLineSize(1),m_uBaseLineColor(RGB(32,32,32))
		,m_uXYAxisFontId(0)
		,m_uXYAxisFontColor(RGB(0,0,255))
		,m_uXMajorScaleLineSize(2)
		,m_uXMinorScaleLineSize(1)
	{
		m_rcChartPadding.left	= 30;
		m_rcChartPadding.top	= 30;
		m_rcChartPadding.right	= 30;
		m_rcChartPadding.bottom	= 60;
	}

	ZQCChartXYAxis::~ZQCChartXYAxis()
	{
		
	}

	void ZQCChartXYAxis::Invalidate()
	{
		if(m_pChartView)
			m_pChartView->Invalidate();
	}

// 	UINT ZQCChartXYAxis::GetXChartLabelsCount()
// 	{
// 		return m_rXChartLabels.size();
// 	}

	UINT ZQCChartXYAxis::GetYChartLabelsCount()
	{
		return m_rYChartLabels.size();
	}
	//!  major scale  minor scale
	//! 每一個刻度点代表Y轴的数值
	//!  rc.left, rc.top, 起点 XY
	//! rc.right, rc.bottom 结束点XY
	void ZQCChartXYAxis::DrawAxis(HDC hDC, const RECT& rcPaint,RECT& rcItem)
	{
		rcItem.left		+= m_rcChartPadding.left;
		rcItem.top		+= m_rcChartPadding.top;
		rcItem.right	-= m_rcChartPadding.right;
		...
		m_nChartWidth = rcItem.right - rcItem.left;
		m_nChartHeight = rcItem.bottom - rcItem.top;

		UINT uXAxisCount,uYAxisCount,uXPerScalePoints,uXTwoPointsInterval;
		uXAxisCount = m_pChartView->GetXScoreCount();
		uYAxisCount = GetYChartLabelsCount();
		uXTwoPointsInterval = m_pChartView->GetXTwoPointsInterval();
		uXPerScalePoints = m_pChartView->GetXPerScalePoints();

		RECT nRc		= rcItem;
		//! 画Y坐标轴
		{
			nRc.right = rcItem.left;
			CRenderEngine::DrawLine(hDC,nRc,m_uXYAxisLineSize,m_uXYAxisLineColor);
		}
		...
		if ( uYAxisCount > 0 )
		{
			m_uYPerChartAxis = m_nChartHeight / uYAxisCount;
			//!  显示Y轴刻度
			for ( size_t i = 1;i <= uYAxisCount ;i ++ )
			{
				nRc = rcItem;
				nRc.bottom -= m_uYPerChartAxis * i;
				nRc.top = nRc.bottom;

				CRenderEngine::DrawLine(hDC,nRc,m_uBaseLineSize,m_uBaseLineColor,PS_DOT);
				nRc.right = nRc.left + 5;
				nRc.left -= 5;
				CRenderEngine::DrawLine(hDC,nRc,2,m_uBaseLineColor);

				nRc = rcItem;
				RECT nRcLegendText		= {0};
				nRcLegendText.left = nRc.left - 25;
				nRcLegendText.right = nRc.left + 60;
				nRcLegendText.top = nRc.bottom - (m_uYPerChartAxis * i * 3) / 2;
				nRcLegendText.bottom = nRc.bottom - (m_uYPerChartAxis * i)/2;

				CRenderEngine::DrawText(hDC,m_pChartView->GetManager(),nRcLegendText,m_rYChartLabels.at(i-1).c_str(),m_uXYAxisFontColor,m_uXYAxisFontId,DT_LEFT|DT_VCENTER|DT_SINGLELINE);
			}
		}
		...
		if ( uXAxisCount > 0 )
		{
			//! 显示X轴刻度
			CDuiString strTemp;
			for ( size_t i = 1;i <= uXAxisCount ;i ++ )
			{
				nRc = rcItem;
				nRc.top = nRc.bottom - 5;
				nRc.bottom += 5;
				nRc.left += uXPerScalePoints*uXTwoPointsInterval*i;
				nRc.right = nRc.left;
				//! 画X轴刻度线
				CRenderEngine::DrawLine(hDC,nRc,m_uXMajorScaleLineSize,m_uBaseLineColor);

				nRc = rcItem;
				RECT nRcLegendText		= {0};
				nRcLegendText.top = nRc.bottom + 5;
				nRcLegendText.bottom = nRc.bottom + 25;
				nRcLegendText.left = nRc.left + (uXPerScalePoints*uXTwoPointsInterval * i)/2;
				nRcLegendText.right = nRc.left + (uXPerScalePoints*uXTwoPointsInterval * i * 3) / 2;
				strTemp.SmallFormat(_T("%d"),uXPerScalePoints*i);
				//! 显示X轴刻度值
				CRenderEngine::DrawText(hDC,m_pChartView->GetManager(),nRcLegendText,strTemp.GetData(),m_uXYAxisFontColor,m_uXYAxisFontId,DT_CENTER|DT_VCENTER|DT_SINGLELINE);
			}

			//! 显示X轴刻度值(时间)
			size_t x = m_pChartView->GetXPerScalePoints() > 0?m_pChartView->GetSeriesShowStartIndex()/m_pChartView->GetXPerScalePoints():0;
			for ( size_t i = 0;i < m_pChartView->GetXScoreCount() && x+i <  m_rXChartLabels.size();i ++ )
			{
				nRc = rcItem;
				RECT nRcLegendText		= {0};
				nRcLegendText.top = nRc.bottom + 35;
				nRcLegendText.bottom = nRc.bottom + 55;
				nRcLegendText.left = nRc.left + (uXPerScalePoints*uXTwoPointsInterval * (i+1))/2;
				nRcLegendText.right = nRc.left + (uXPerScalePoints*uXTwoPointsInterval * (i+1) * 3) / 2;
				CRenderEngine::DrawText(hDC,m_pChartView->GetManager(),nRcLegendText,m_rXChartLabels.at(x+i).c_str(),m_uXYAxisFontColor,m_uXYAxisFontId,DT_CENTER|DT_VCENTER|DT_SINGLELINE);
			}
			//! 显示X轴小坐标
			if ( uXTwoPointsInterval >= 5 )
				for ( size_t i = 1;uXTwoPointsInterval*i < m_nChartWidth /*m_uXPerScalePoints*m_uXAxisCount*/;i ++ )
				{
					nRc = rcItem;
					//
					//nRc.bottom += 5;
					nRc.left += uXTwoPointsInterval*i;
					nRc.right = nRc.left;
					//! 画刻度线
					if( m_pChartView->GetVisibleGridlines() )
					{
						CRenderEngine::DrawLine(hDC,nRc,m_uXMinorScaleLineSize,m_uBaseLineColor,PS_DOT);
					}
					else
					{
						nRc.top = nRc.bottom - 3;
						CRenderEngine::DrawLine(hDC,nRc,m_uXMinorScaleLineSize,m_uBaseLineColor);
					}		
				}
		}

		//! 画连接线
		if ( uXAxisCount > 0 && uYAxisCount > 0 && m_pChartView->GetSeriesCount() > 0 )
		{
			POINT currentPoint = {0,0},prevPoint = {0,0};
			size_t i = m_pChartView->GetSeriesShowStartIndex();
			float fData = 0.00;
			m_fYPerAxis = m_uYPerChartAxis / m_pChartView->GetYMajorScaleUnit() ;
		
			for ( size_t x = 0;i < m_pChartView->GetSeriesCount(); i ++ ,x ++)
			{
				fData = m_pChartView->GetSeriesData( i );
				currentPoint.x = rcItem.left + uXTwoPointsInterval*(x+1);
				currentPoint.y = static_cast<LONG>(rcItem.bottom - m_uYPerChartAxis*fData / m_pChartView->GetYMajorScaleUnit());
#if 0
				TCHAR szBuffer[26] = {0};
				_stprintf_s(szBuffer,_T("%.2f\n"),fData);
				OutputDebugString( szBuffer );
#endif
				if ( prevPoint.x != 0 )
				{
					nRc.left = prevPoint.x;
					nRc.top = prevPoint.y;
					nRc.right = currentPoint.x;
					nRc.bottom = currentPoint.y;
					CRenderEngine::DrawLine(hDC,nRc,m_nLineWidth,m_uLineColor);
				}
	...
	void ZQCChartXYAxis::SetChartViewObj(CZQChartViewUI* _pChartViewUI)
	{
		m_pChartView = _pChartViewUI;
	}

	bool ZQCChartXYAxis::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		 if( _tcscmp(pstrName, _T("linewidth")) == 0 ) m_nLineWidth = _ttoi(pstrValue);
		 else if( _tcscmp(pstrName, _T("linecolor")) == 0 )
		 {
			 while( *pstrValue > _T('\0') && *pstrValue <= _T(' ') ) pstrValue = ::CharNext(pstrValue);
			 if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			 LPTSTR pstr = NULL;
			 m_uLineColor = _tcstoul(pstrValue, &pstr, 16);
		 }
		 else if( _tcscmp(pstrName, _T("fontid")) == 0 ) m_uXYAxisFontId = _ttoi(pstrValue);
		 else if( _tcscmp(pstrName, _T("fontcolor")) == 0 )
		 {
			 while( *pstrValue > _T('\0') && *pstrValue <= _T(' ') ) pstrValue = ::CharNext(pstrValue);
			 if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			 LPTSTR pstr = NULL;
			 m_uXYAxisFontColor = _tcstoul(pstrValue, &pstr, 16);
		 }
		 else if( _tcscmp(pstrName, _T("xyaxislinesize")) == 0 ) m_uXYAxisLineSize = _ttoi(pstrValue);
		 else if( _tcscmp(pstrName, _T("xyaxislinecolor")) == 0 )
		 {
			 while( *pstrValue > _T('\0') && *pstrValue <= _T(' ') ) pstrValue = ::CharNext(pstrValue);
			 if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			 LPTSTR pstr = NULL;
			 m_uXYAxisLineColor = _tcstoul(pstrValue, &pstr, 16);
		 }
		 else return false;
	...
	/**************************************/
	CZQChartViewUI::CZQChartViewUI( void ):m_cXYAxis(m_rXChartLabels,m_rYChartLabels)
		,m_dShowStartIndex(0)
		,m_uXTwoPointsInterval(10)
		,m_xScoreCount(5)
		,m_uXPerScalePoints(0)
		,m_uMaxXPointsCount(0)
		,m_bVisibleGridlines(false)
		,m_uMaxOldXPointsCount(0)
	{
		m_cXYAxis.SetChartViewObj(this);
	}

	CZQChartViewUI::~CZQChartViewUI()
	{

	}

	bool CZQChartViewUI::GetVisibleGridlines()
	{
		return m_bVisibleGridlines;
	}

	void CZQChartViewUI::SetVisibleGridlines( bool bVisibleGridlines )
	{
		m_bVisibleGridlines = bVisibleGridlines;
		Invalidate();
	}

	LPCTSTR CZQChartViewUI::GetClass() const
	{
		return _T("ZQChartViewUI");
	}

	LPVOID CZQChartViewUI::GetInterface( LPCTSTR pstrName )
	{
		if( _tcscmp(pstrName, DUI_CTR_ZQ_CHARTVIEW) == 0 ) return static_cast<CZQChartViewUI*>(this);
		else return CHorizontalLayoutUI::GetInterface(pstrName);
	}

	void CZQChartViewUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if ( !m_cXYAxis.SetAttribute(pstrName,pstrValue) )
			CHorizontalLayoutUI::SetAttribute(pstrName, pstrValue);
	}

	void CZQChartViewUI::RemoveLabel()
	{
		m_rXChartLabels.clear();
		Invalidate();
	}

	void CZQChartViewUI::RemoveYLabel()
	{
		m_rYChartLabels.clear();
		Invalidate();
	}

	void CZQChartViewUI::AddLabel( LPCTSTR _DataLabel )
	{
		m_rXChartLabels.push_back(_DataLabel);
		Invalidate();
	}

	void CZQChartViewUI::AddYLabel( LPCTSTR _DataLabel )
	{
		m_rYChartLabels.push_back(_DataLabel);
		Invalidate();
	}
	
	void CZQChartViewUI::MoveSeriesShowStartIndex( bool bLeft )
	{
		UINT uMax = /*GetXScoreCount() **/ GetXPerScalePoints();
		if ( bLeft )
		{
			if ( m_dShowStartIndex <= 0  || m_dShowStartIndex <= uMax )
			{
				m_dShowStartIndex = 0;
			}
			else if ( m_dShowStartIndex >= GetSeriesCount() )
			{
				m_dShowStartIndex = GetSeriesCount() - uMax;
			}
			else if ( m_dShowStartIndex > uMax )
			{
				m_dShowStartIndex -= uMax;
			}
		}
		else
		{
			if ( m_dShowStartIndex >= GetSeriesCount() || GetSeriesCount() - m_dShowStartIndex ==  uMax )
			{
				m_dShowStartIndex = GetSeriesCount() - uMax;
			}
			else if ( GetSeriesCount() - m_dShowStartIndex <  uMax )
			{
				//m_dShowStartIndex = GetSeriesCount() - uMax;
			}
			else if (  GetSeriesCount() - m_dShowStartIndex >  uMax )
	...
	void CZQChartViewUI::SetSeriesShowStartIndex( UINT nIndex )
	{
		if ( nIndex < 0  || m_dShowStartIndex >= GetSeriesCount() ) nIndex = 0;
		m_dShowStartIndex = nIndex;
		Invalidate();
	}

	UINT CZQChartViewUI::GetSeriesShowStartIndex()
	{
		if (  m_uMaxOldXPointsCount != m_uMaxXPointsCount )
		{
			ResetSeriesShowStartIndex();
			m_uMaxOldXPointsCount = m_uMaxXPointsCount;
		}
		return m_dShowStartIndex;
	}

	void CZQChartViewUI::SetYMajorScaleUnit( float dData )
	{
		m_fMajorScaleUnit = dData;
	}

	float CZQChartViewUI::GetYMajorScaleUnit()
	{
		return m_fMajorScaleUnit;
	}

	void CZQChartViewUI::ClearSeriesData()
	{
		m_dShowStartIndex = 0;
		m_rSeriesData.clear();
		Invalidate();
	...
	UINT CZQChartViewUI::GetMaxXPointsCount()
	{
		if ( m_uMaxXPointsCount != GetXScoreCount() * GetXPerScalePoints() ) m_uMaxOldXPointsCount = m_uMaxXPointsCount;
		m_uMaxXPointsCount =  GetXScoreCount() * GetXPerScalePoints();
		return m_uMaxXPointsCount;
	}

	void CZQChartViewUI::ResetSeriesShowStartIndex()
	{		
		m_dShowStartIndex = GetSeriesCount() > GetMaxXPointsCount()?GetSeriesCount() - GetMaxXPointsCount():0;
		Invalidate();
	}

	void CZQChartViewUI::AddSeriesData(float _DataValue)
	{
		m_rSeriesData.push_back(_DataValue);
		if ( GetSeriesCount() > GetMaxXPointsCount() ) m_dShowStartIndex++;
		Invalidate();
	}

	float CZQChartViewUI::GetSeriesData( int nIndex )
	{
		return m_rSeriesData.at(nIndex);
	}

	UINT CZQChartViewUI::GetSeriesCount()
	{
		return m_rSeriesData.size();
	}

	void CZQChartViewUI::SetXTwoPointsInterval( UINT xTwoPointsInterval )
	{
		m_uXTwoPointsInterval = xTwoPointsInterval;
	}

	UINT CZQChartViewUI::GetXTwoPointsInterval()
	{
		return m_uXTwoPointsInterval;
	}

	UINT CZQChartViewUI::GetXPerScalePoints()
	{
		return (m_sScoreRect.right - m_sScoreRect.left)/m_uXTwoPointsInterval/m_xScoreCount;
	}

	UINT CZQChartViewUI::GetXScoreCount()
	{
		return m_xScoreCount;
	}

	void CZQChartViewUI::SetXScoreCount( UINT xScoreCount )
	{
		m_xScoreCount = xScoreCount;
	//	m_uMaxXPointsCount = m_xScoreCount * GetXPerScalePoints();
	}

	void CZQChartViewUI::DoPaint( HDC hDC, const RECT& rcPaint )
	{
		if( !::IntersectRect(&m_rcPaint, &rcPaint, &m_rcItem) ) return;
		CHorizontalLayoutUI::DoPaint(hDC,rcPaint);
		m_sScoreRect = m_rcItem;
		m_cXYAxis.DrawAxis(hDC,rcPaint,m_sScoreRect);
		/*if( 1/ *!m_rXChartLabels.IsEmpty() || !m_rYChartLabels.IsEmpty()* / ){*/
