...
namespace UiLib {
    ...
    CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false)/*, m_iCurSel(-1)*/, m_iExpandedItem(-1),m_bSingleSel(false),m_bNeedEndDown(false),m_bNeedLineDown(false)
    {
    ...
    CListUI::~CListUI()
    {
        for (int i = 0;i < m_aIdelListTextElementUi.GetSize();i ++)
        {
            delete static_cast<CControlUI*>(m_aIdelListTextElementUi[i]);
        }
    }
    ...
    void CListUI::SetNeedLineDown(bool bAutoLineDown)
    {
        m_bNeedLineDown = bAutoLineDown;
        NeedUpdate();
        SendMessage(m_pManager->GetPaintWindow(),WM_PAINT,NULL,NULL);
    }

    void CListUI::SetNeedEndDown(bool bAutoEndDown)
    {
        m_bNeedEndDown = bAutoEndDown;
    }
    ...
    int CListUI::GetItemIndex(CControlUI* pControl) const
    {
        ...
        if( _tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM) != NULL || _tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM_CONTAINER) != NULL ) return m_pHeader->GetItemIndex(pControl);
    ...
    bool CListUI::SetItemIndex(CControlUI* pControl, int iIndex)
    {
        ...
        // We also need to recognize header sub-items
        if( _tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM) != NULL || _tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM_CONTAINER) != NULL ) return m_pHeader->SetItemIndex(pControl, iIndex);
    ...
    bool CListUI::Add(CControlUI* pControl)
    {
        ...
        // We also need to recognize header sub-items
        if( _tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM) != NULL || _tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM_CONTAINER) != NULL ) {
    ...
    bool CListUI::Remove(CControlUI* pControl)
    {
        ...
        // We also need to recognize header sub-items
        if( _tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM ) != NULL || _tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM_CONTAINER) != NULL ) return m_pHeader->Remove(pControl);
    ...
    void CListUI::RemoveAllShow()
    {
        for (int i = 0;i < GetCount();i ++)
        {
            m_aIdelListTextElementUi.Add(m_pList->GetItemAt(i));
        }
        m_iExpandedItem = -1;
        m_pList->CContainerUI::RemoveAllShow();
    }
    ...
    {         
        if (m_pList->GetCount() > 0)
        {
            if (m_bNeedEndDown) m_pList->EndDown();
            if (m_bNeedLineDown) m_pList->LineDown();
            m_bNeedEndDown = false;
            m_bNeedLineDown = false;
        }
    ...
    {
        if( _tcscmp(pstrName, DUI_CTR_LISTITEM) == 0 ) return static_cast<IListItemUI*>(this);
        if( _tcscmp(pstrName, DUI_CTR_LISTCONTAINERELEMENT_EX) == 0 ) return static_cast<CListContainerElementUIEx*>(this);
        return CContainerUI::GetInterface(pstrName);
    }
    ...
    bool CListContainerElementUIEx::Select(bool bSelect)
    {
        if( !IsEnabled() ) return false;
        if( bSelect == m_bSelected ) return true;
        m_bSelected = bSelect;
    ...
    {	
        ...
        CListUI* pList = static_cast<CListUI*>(m_pOwner);
        if (pList == NULL ||  (_tcscmp(_T("ListUI"), pList->GetClass()) != 0 && _tcscmp(_T("ListTestCtrlUI"), pList->GetClass()) != 0)) return;
        ...
        for (int i = 0; i < nCount; i++)
        {
            ...
            {
                CListHeaderItemContainer *pHeaderItem = static_cast<CListHeaderItemContainer*>(m_pHeader->GetItemAt(i));
    ...
    CListHeaderItemContainer::CListHeaderItemContainer() : m_bDragable(true), m_uButtonState(0), m_iSepWidth(4),
    ...
    LPCTSTR CListHeaderItemContainer::GetClass() const
    {
        return _T("ListHeaderItemContainer");
    }

    LPVOID CListHeaderItemContainer::GetInterface(LPCTSTR pstrName)
    {
        if( _tcscmp(pstrName, DUI_CTR_LISTHEADERITEM_CONTAINER) == 0 ) return this;
        return CContainerUI::GetInterface(pstrName);
    }

    UINT CListHeaderItemContainer::GetControlFlags() const
    {
        if( IsEnabled() && m_iSepWidth != 0 ) return UIFLAG_SETCURSOR;
        else return 0;
    }

    void CListHeaderItemContainer::SetEnabled(bool bEnable)
    {
        CControlUI::SetEnabled(bEnable);
        if( !IsEnabled() ) {
            m_uButtonState = 0;
    ...
    bool CListHeaderItemContainer::IsDragable() const
    {
        return m_bDragable;
    }

    void CListHeaderItemContainer::SetDragable(bool bDragable)
    {
        m_bDragable = bDragable;
        if ( !m_bDragable ) m_uButtonState &= ~UISTATE_CAPTURED;
    }

    DWORD CListHeaderItemContainer::GetSepWidth() const
    {
        return m_iSepWidth;
    }

    void CListHeaderItemContainer::SetSepWidth(int iWidth)
    {
        m_iSepWidth = iWidth;
    }

    DWORD CListHeaderItemContainer::GetTextStyle() const
    {
        return m_uTextStyle;
    }

    void CListHeaderItemContainer::SetTextStyle(UINT uStyle)
    {
        m_uTextStyle = uStyle;
        Invalidate();
    }

    DWORD CListHeaderItemContainer::GetTextColor() const
    {
        return m_dwTextColor;
    }


    void CListHeaderItemContainer::SetTextColor(DWORD dwTextColor)
    {
        m_dwTextColor = dwTextColor;
    }

    RECT CListHeaderItemContainer::GetTextPadding() const
    {
        return m_rcTextPadding;
    }

    void CListHeaderItemContainer::SetTextPadding(RECT rc)
    {
        m_rcTextPadding = rc;
        Invalidate();
    }

    void CListHeaderItemContainer::SetFont(int index)
    {
        m_iFont = index;
    }

    bool CListHeaderItemContainer::IsShowHtml()
    {
        return m_bShowHtml;
    }

    void CListHeaderItemContainer::SetShowHtml(bool bShowHtml)
    {
        if( m_bShowHtml == bShowHtml ) return;
    ...
    LPCTSTR CListHeaderItemContainer::GetNormalImage() const
    {
        return m_sNormalImage;
    }

    void CListHeaderItemContainer::SetNormalImage(LPCTSTR pStrImage)
    {
        m_sNormalImage = pStrImage;
        Invalidate();
    }

    LPCTSTR CListHeaderItemContainer::GetHotImage() const
    {
        return m_sHotImage;
    }

    void CListHeaderItemContainer::SetHotImage(LPCTSTR pStrImage)
    {
        m_sHotImage = pStrImage;
        Invalidate();
    }

    LPCTSTR CListHeaderItemContainer::GetPushedImage() const
    {
        return m_sPushedImage;
    }

    void CListHeaderItemContainer::SetPushedImage(LPCTSTR pStrImage)
    {
        m_sPushedImage = pStrImage;
        Invalidate();
    }

    LPCTSTR CListHeaderItemContainer::GetFocusedImage() const
    {
        return m_sFocusedImage;
    }

    void CListHeaderItemContainer::SetFocusedImage(LPCTSTR pStrImage)
    {
        m_sFocusedImage = pStrImage;
        Invalidate();
    }

    LPCTSTR CListHeaderItemContainer::GetSepImage() const
    {
        return m_sSepImage;
    }

    void CListHeaderItemContainer::SetSepImage(LPCTSTR pStrImage)
    {
        m_sSepImage = pStrImage;
        Invalidate();
    }

    void CListHeaderItemContainer::SetScale(int nScale)
    {
        m_nScale = nScale;
    }

    int CListHeaderItemContainer::GetScale() const
    {
        return m_nScale;
    }

    void CListHeaderItemContainer::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
    {
        if( _tcscmp(pstrName, _T("dragable")) == 0 ) SetDragable(_tcscmp(pstrValue, _T("true")) == 0);
        else if( _tcscmp(pstrName, _T("sepwidth")) == 0 ) SetSepWidth(_ttoi(pstrValue));
        else if( _tcscmp(pstrName, _T("align")) == 0 ) {
    ...
    void CListHeaderItemContainer::DoEvent(TEventUI& event)
    {
        if( !IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND ) {
            if( m_pParent != NULL ) m_pParent->DoEvent(event);
            else CContainerUI::DoEvent(event);
    ...
    SIZE CListHeaderItemContainer::EstimateSize(SIZE szAvailable)
    {
        if( m_cxyFixed.cy == 0 ) return CSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 14);
        return CContainerUI::EstimateSize(szAvailable);
    }

    RECT CListHeaderItemContainer::GetThumbRect() const
    {
        if( m_iSepWidth >= 0 ) return CDuiRect(m_rcItem.right - m_iSepWidth, m_rcItem.top, m_rcItem.right, m_rcItem.bottom);
        else return CDuiRect(m_rcItem.left, m_rcItem.top, m_rcItem.left - m_iSepWidth, m_rcItem.bottom);
    }

    void CListHeaderItemContainer::PaintStatusImage(HDC hDC)
    {
        if( IsFocused() ) m_uButtonState |= UISTATE_FOCUSED;
        else m_uButtonState &= ~ UISTATE_FOCUSED;
    ...
    void CListHeaderItemContainer::PaintText(HDC hDC)
    {
        if( m_dwTextColor == 0 ) m_dwTextColor = m_pManager->GetDefaultFontColor();
