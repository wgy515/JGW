		...
		for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
			...
			//树控件XML解析
				...
				case 6:
					...
					break;
				case 7:
                    if( _tcscmp(pstrClass, DUI_CTR_BARCODE) == 0 )                pControl = new CBarcodeUI;
					else if( _tcscmp(pstrClass, DUI_CTR_CONTROL) == 0 )                pControl = new CControlUI;
				...
				case 10:
					...
					else if( _tcscmp(pstrClass, DUI_CTR_PROJECTBTN) == 0 )        pControl = new CProjectBtnUI;
					break;
				case 11:
					if (_tcscmp(pstrClass, DUI_CTR_CHILDLAYOUT) == 0)			  pControl = new CChildLayoutUI;
					else if (_tcscmp(pstrClass, DUI_CTR_ZQ_CHARTVIEW) == 0)			  pControl = new CZQChartViewUI;
					else if (_tcscmp(pstrClass, DUI_CTR_PROJECTLIST) == 0)			  pControl = new CProjectListUI;
					break;
				case 12:
					 if( _tcscmp(pstrClass, DUI_CTR_LIST_TEST_CTRL) == 0 )    pControl = new CListTestCtrlUI;
				...
				case 22:
					if( _tcscmp(pstrClass, DUI_CTR_LISTCONTAINERELEMENT_EX) == 0 )   pControl = new CListContainerElementUIEx;
				case 23:
					if( _tcscmp(pstrClass, DUI_CTR_LISTHEADERITEM_CONTAINER) == 0 )     pControl = new CListHeaderItemContainer;
				case 24:
					if( _tcscmp(pstrClass, DUI_CTR_VERTICALLAYOUT_PROJECT_BUTTON) == 0 )     pControl = new CUIVerticalLayoutUIProjectBtn;
					break;
				}
