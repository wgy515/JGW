...
namespace UiLib
{
	...
	CControlUI::CControlUI() : m_pManager(NULL),m_nScale(0), 
		m_pParent(NULL), 
	...
	LPVOID CControlUI::GetDate()
	{
		return m_lpDate;
	}

	void CControlUI::SetDate(LPVOID lpvoid)
	{
		m_lpDate = lpvoid;
	}
	...
	void CControlUI::SetScaleHeader(bool bIsScale)
	{
		m_bIsScaleHeader = bIsScale;
	}

	bool CControlUI::IsScaleHeader() const
	{
	...
	void CControlUI::SetScale(int nScale)
	{
		m_nScale = nScale;
	}
	
	int CControlUI::GetScale() const
	{
		return m_nScale;
	}

	int CControlUI::GetScale( int nParentWidth ) const
	{
		static int g_nPreParentWidth = 0;
		if ( 0 == g_nPreParentWidth ) g_nPreParentWidth = nParentWidth;
		if ( 0 == GetWidth() ) return m_nScale;

		if ( nParentWidth != g_nPreParentWidth )
		{
			int iScale = GetWidth()*100 / g_nPreParentWidth ;
			g_nPreParentWidth = nParentWidth;
			return iScale;
		}
		return /*m_nScale;*/GetWidth()*100 / nParentWidth ;
	}
	...
	void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("scalelayout")) == 0 ) SetScaleHeader(_tcscmp(pstrValue, _T("true")) == 0);
	...
	{
		static CDuiString g_strAttribute;
		...
		{
			g_strAttribute = GetBkImage();
			nRetData.pcwchar = g_strAttribute.GetData();
		}	
