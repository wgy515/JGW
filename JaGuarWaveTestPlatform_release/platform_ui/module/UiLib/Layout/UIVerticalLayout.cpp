...
namespace UiLib
{
	...
	CVerticalLayoutUI::CVerticalLayoutUI() : m_iSepHeight(0), m_uButtonState(0), m_bImmMode(false),m_iSepWidth(0)
	{
	...
	{
		if( IsEnabled() && ( m_iSepHeight != 0 ||m_iSepWidth !=0) ) return UIFLAG_SETCURSOR;
		else return 0;
	}
...
//			cyFixedRemaining -= m_iChildPadding;
	...
	void CVerticalLayoutUI::DoPostPaint(HDC hDC, const RECT& rcPaint)
	{
		if( (m_uButtonState & UISTATE_CAPTURED) != 0 && !m_bImmMode ) {
			RECT rcSeparator = GetThumbHeightRect(true);
	...
	void CVerticalLayoutUI::SetSepWidth(int iWeight)
	{
		m_iSepWidth = iWeight;
	}

	int CVerticalLayoutUI::GetSepWidth() const
	{
		return m_iSepWidth;
	}
	...
	void CVerticalLayoutUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("sepweight")) == 0 ) SetSepWidth(_ttoi(pstrValue));
	...
	void CVerticalLayoutUI::DoEvent(TEventUI& event)
	{
		if( m_iSepHeight != 0 ) {
			...
			{
				RECT rcSeparator = GetThumbHeightRect(false);
			...
			if( event.Type == UIEVENT_MOUSEMOVE )
			{
				if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
					...
					CDuiRect rcInvalidate = GetThumbHeightRect(true);
					m_rcNewPos = rc;
					...
					if( m_bImmMode ) {
						...
						rcInvalidate.Join(GetThumbHeightRect(true));
						rcInvalidate.Join(GetThumbHeightRect(false));
		...
		if( m_iSepWidth != 0 ) {
			if( event.Type == UIEVENT_BUTTONDOWN && IsEnabled() )
			{
				RECT rcSeparator = GetThumbWidthRect(false);
				if( ::PtInRect(&rcSeparator, event.ptMouse) ) {
					m_uButtonState |= UISTATE_CAPTURED;
					ptLastMouse = event.ptMouse;
					m_rcNewPos = m_rcItem;
...
// 						}
					...
					CDuiRect rcInvalidate = GetThumbWidthRect(true);
					m_rcNewPos = rc;
					//m_cxyFixed.cy = m_rcNewPos.bottom - m_rcNewPos.top;
					...
					if( m_bImmMode ) {
						...
						rcInvalidate.Join(GetThumbWidthRect(true));
						rcInvalidate.Join(GetThumbWidthRect(false));
						if( m_pManager ) m_pManager->Invalidate(rcInvalidate);
					}
					return;
				}
...
//			}
	...
	RECT CVerticalLayoutUI::GetThumbHeightRect(bool bUseNew) const
	{
	...
	RECT CVerticalLayoutUI::GetThumbWidthRect(bool bUseNew) const
	{
		if( (m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
