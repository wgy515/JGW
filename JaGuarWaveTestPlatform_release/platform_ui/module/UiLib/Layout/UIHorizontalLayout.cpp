...
namespace UiLib
{
	...
	CHorizontalLayoutUI::CHorizontalLayoutUI() : m_iSepWidth(0), m_uButtonState(0), m_bImmMode(false),m_iSepHeight(0)
	{
	...
	{
		if( IsEnabled() && ( m_iSepWidth != 0 || m_iSepHeight != 0) ) return UIFLAG_SETCURSOR;
		else return 0;
	}
	...
	void CHorizontalLayoutUI::SetPos(RECT rc)
	{
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if ( m_bIsScaleHeader )
			{
				CControlUI* pVerticalLayoutUI = static_cast<CControlUI*>(pControl);
				if ( m_items.GetSize() != 2 )
					sz.cx = int((m_rcItem.right-m_rcItem.left) * (float)pVerticalLayoutUI->GetScale( m_rcItem.right-m_rcItem.left ) / 100);
				else
				{
					if ( 0 == it2 )
					{
						iPreSale = pVerticalLayoutUI->GetScale( m_rcItem.right-m_rcItem.left );
						sz.cx = int((m_rcItem.right-m_rcItem.left) * (float)iPreSale / 100);
					}	
					else if ( it2 == 1 )
					{
						sz.cx = int((m_rcItem.right-m_rcItem.left) * (100.00 - iPreSale)/100);
					}
				}
			}
			else
...
//			cxFixedRemaining -= m_iChildPadding;
	...
	void CHorizontalLayoutUI::DoPostPaint(HDC hDC, const RECT& rcPaint)
	{
		if( (m_uButtonState & UISTATE_CAPTURED) != 0 && !m_bImmMode ) {
			RECT rcSeparator = GetThumbRectWidth(true);
			CRenderEngine::DrawColor(hDC, rcSeparator, 0xAA000000);
		}
	}

	void CHorizontalLayoutUI::SetSepHeight(int iHeight)
	{
	...
	void CHorizontalLayoutUI::DoEvent(TEventUI& event)
	{
		if( m_iSepWidth != 0 ) {
			...
			{
				RECT rcSeparator = GetThumbRectWidth(false);
			...
			if( event.Type == UIEVENT_MOUSEMOVE )
			{
				if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
					...
					CDuiRect rcInvalidate = GetThumbRectWidth(true);
					m_rcNewPos = rc;
					...
					if( m_bImmMode ) {
						...
						rcInvalidate.Join(GetThumbRectWidth(true));
						rcInvalidate.Join(GetThumbRectWidth(false));
		...
		if( m_iSepHeight != 0 ){
			if( event.Type == UIEVENT_BUTTONDOWN && IsEnabled() )
			{
				RECT rcSeparator = GetThumbRectHeigth(false);
				if( ::PtInRect(&rcSeparator, event.ptMouse) ) {
					m_uButtonState |= UISTATE_CAPTURED;
					ptLastMouse = event.ptMouse;
					m_rcNewPos = m_rcItem;
			...
			if( event.Type == UIEVENT_MOUSEMOVE )
			{
				if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
					...
					RECT rc = m_rcNewPos;
					if( m_iSepHeight >= 0 ) {
						if( cy > 0 && event.ptMouse.y < m_rcNewPos.top - m_iSepWidth ) return;
						...
						if( cy < 0 && event.ptMouse.y > m_rcNewPos.bottom + m_iSepWidth ) return;
						rc.bottom += cy;
                        //! 如果宽度小于最小高度
                        if (rc.bottom - rc.top <= GetMinHeight()) {
                            if( m_rcNewPos.bottom - m_rcNewPos.top <= GetMinHeight() ) return;
                        ...
                        if( rc.bottom - rc.top >= GetMaxWidth() ) {
                            if( m_rcNewPos.bottom - m_rcNewPos.top >= GetMaxHeight() ) return;
                            rc.bottom = rc.top - GetMaxHeight();
                        }
						//if( rc.top - rc.bottom <= GetMinHeight() ) {
						//	//if( m_rcNewPos.top - m_rcNewPos.bottom <= GetMinHeight() ) return;
						//	rc.bottom = rc.top - GetMinHeight();
						//}
						//if( rc.top - rc.bottom >= GetMaxWidth() ) {
						//	//if( m_rcNewPos.top - m_rcNewPos.bottom >= GetMaxHeight() ) return;
						//	rc.bottom = rc.top - GetMaxHeight();
						//}
					}

					CDuiRect rcInvalidate = GetThumbRectHeigth(true);
					m_rcNewPos = rc;
					...
					if( m_bImmMode ) {
						...
						rcInvalidate.Join(GetThumbRectHeigth(true));
						rcInvalidate.Join(GetThumbRectHeigth(false));
						if( m_pManager ) m_pManager->Invalidate(rcInvalidate);
					}
					return;
				}
	...
	RECT CHorizontalLayoutUI::GetThumbRectWidth(bool bUseNew) const
	{
	...
	RECT CHorizontalLayoutUI::GetThumbRectHeigth(bool bUseNew /* = false */) const 
	{
		if( (m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
			if( m_iSepHeight >= 0 ) return CDuiRect(m_rcNewPos.left , m_rcNewPos.top - m_iSepHeight, m_rcNewPos.right, m_rcNewPos.top +m_iSepHeight );
			else return CDuiRect(m_rcNewPos.left, m_rcNewPos.bottom + m_iSepHeight, m_rcNewPos.right, m_rcNewPos.bottom - m_iSepHeight );
		}
		else {
			if( m_iSepHeight >= 0 ) return CDuiRect(m_rcItem.left , m_rcItem.top - m_iSepHeight, m_rcItem.right, m_rcItem.top + m_iSepHeight + m_iSepHeight );
			else return CDuiRect(m_rcItem.left, m_rcItem.bottom + m_iSepHeight, m_rcItem.right, m_rcItem.bottom - m_iSepHeight );
		}
	}
}
