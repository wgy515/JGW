...
namespace UiLib
{
    ...
    {
#if 1
        HWND hTrayWndHandle = FindWindowEx(NULL, NULL,L"Shell_TrayWnd",NULL);
        HWND hTrayNotifyWndHandle = FindWindowEx(hTrayWndHandle, NULL, L"TrayNotifyWnd", NULL);
        HWND hSysPagerHandle = FindWindowEx(hTrayNotifyWndHandle, NULL, L"SysPager", NULL);
        HWND hNotifyAreaHandle = FindWindowEx(hSysPagerHandle, NULL, L"ToolbarWindow32", NULL);
        if (hNotifyAreaHandle != NULL) RefreshWindow(hNotifyAreaHandle);
        HWND hOverHandle = FindWindowEx(NULL, NULL, L"NotifyIconOverflowWindow", NULL);
        HWND hNotifyOverHandle = FindWindowEx(hOverHandle,NULL, L"ToolbarWindow32", NULL);
        if (hNotifyOverHandle != NULL) RefreshWindow(hNotifyOverHandle);
#endif
        if(_Message == NULL)
