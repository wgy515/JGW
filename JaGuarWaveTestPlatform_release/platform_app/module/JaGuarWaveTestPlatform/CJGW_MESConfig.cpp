...
namespace JGW
{
    ...
    bool CCJGW_MESConfig::LoadMESIniConfig(const std::wstring& strMesConfigPath,LPS_MES_INFO psMesInfo)
    {
		...
		psMesInfo->mstrFTPLogBackGroundExec = configIni.GetIniKeyValue(L"FTPLogBackGround",L"exec_name",L"");
        ...
        Log4WD_F(L"MESServiceIP: %s,MESServiceSocketPort: %d",psMesInfo->msMesConnectInfo.mstrMESServiceIP.c_str(),psMesInfo->msMesConnectInfo.mnMESServiceSocketPort);
        ...
        Log4WD_F(L"UserName: %s,UserPassword: %s",psMesInfo->msMesConnectInfo.mstrUsername.c_str(),strTemp.c_str());
    ...
    bool CCJGW_MESConfig::LoadMESWorkStageXmlConfig(const std::wstring& strMesXmlConfigPath,LPS_MES_INFO psMesInfo)
    {
        ...
        Log4WD_F(L"LoadMESWorkStageXmlConfig : %s",strMesXmlConfigPath.c_str());
        ...
        while (pWorkstageNode)
        {
            ...
            Log4WD_F(L"id: %s,name: %s,des: %s",strTemp.c_str(),sMesWorkStage.mstrWorkStageName.c_str(),sMesWorkStage.mstrWorkStageDes.c_str());
    ...
    std::string CCJGW_MESConfig::EncryptPassword(const std::string& strPassword)
    {
        ...
        {
            PELOG4WW(GetString(E_PASSWORD_ENCRYPTION_FAILED_TYPE));
        } 
        return encrypted;
    }
    ...
    std::string CCJGW_MESConfig::DecryptPassword(const std::string& strEncryptPassword)
    {
        ...
        {
            //PELOG4WW(L"√‹¬ÎΩ‚√‹ ß∞‹");
            PELOG4WW(GetString(E_PASSWORD_DECRYPTION_FAILED_TYPE));
        } 
        return decrypted;
    }
}