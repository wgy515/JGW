...
namespace JGW
{
    ...
    bool CCJGW_HelpSqlite3::GetOTAToOTAID(CCJGW_SQLite& sqliteDB,S_TASK_SQL& sTaskSQL)
    {
        ...
        sTaskSQL.mnOTAID = _ttoi(JGW_A2W(mapFiledsNameVal["ID"],CP_UTF8).c_str());
    ...
    bool CCJGW_HelpSqlite3::GetConfigToConfigID(CCJGW_SQLite& sqliteDB,S_TASK_SQL& sTaskSQL)
    {
        ...
        JGW_FormatString(strSelectTaskSQL,"SELECT PROJECT.NAME AS PROJECT_NAME,PROJECT.DES AS PROJECT_DES,CONFIG.NAME AS CONFIG_NAME,CONFIG.DES AS CONFIG_DES,CONFIG.MD5 AS CONFIG_MD5,CONFIG.FTPURL AS CONFIG_FTPURL,CONFIG.Version AS CONFIG_VERSION FROM CONFIG,PROJECT WHERE PROJECT.ID=CONFIG.ProjectID AND CONFIG.ID=%d",sTaskSQL.mnConfigID);
        ...
        sTaskSQL.msConfig.mstrVersion = JGW_A2W(mapFiledsNameVal["CONFIG_VERSION"],CP_UTF8);
    ...
    bool CCJGW_HelpSqlite3::GetTaskConfigToTaskConfigID(CCJGW_SQLite& sqliteDB,S_TASK_SQL& sTaskSQL)
    {
        ...
        bool bIsUploadLog = false;
        bool bIsUploadTestLog = true;
        std::wstring strCustomLogDir = L"";

        //! 检查IsUploadLog是否是当前表列
        if (mapFiledsNameVal.end() != mapFiledsNameVal.find("IsUploadLog"))
        {
            bIsUploadLog = (1 == atoi(mapFiledsNameVal["IsUploadLog"].c_str()));
        }
        //! 检查IsUploadTestLog是否是当前表列
        if (mapFiledsNameVal.end() != mapFiledsNameVal.find("IsUploadTestLog"))
        {
            bIsUploadTestLog = (1 == atoi(mapFiledsNameVal["IsUploadTestLog"].c_str()));
        }
        //! 检查CustomLogDir是否是当前表列
        if (mapFiledsNameVal.end() != mapFiledsNameVal.find("CustomLogDir"))
        {
            strCustomLogDir = JGW_A2W(mapFiledsNameVal["CustomLogDir"],CP_UTF8);
        }

        if (sTaskSQL.msTaskConfig.mbIsUploadLog != bIsUploadLog 
            || sTaskSQL.msTaskConfig.mbIsUploadTestLog != bIsUploadTestLog
            || 0 != JGW_WStrComparenoCaseWStr(sTaskSQL.msTaskConfig.mstrCustomLogDir.c_str(),strCustomLogDir.c_str()))
        {
            sTaskSQL.msTaskConfig.mbIsUpdateDBLogConfig = true;
            sTaskSQL.msTaskConfig.mbIsUploadLog = bIsUploadLog;
            sTaskSQL.msTaskConfig.mbIsUploadTestLog = bIsUploadTestLog;
            sTaskSQL.msTaskConfig.mstrCustomLogDir = strCustomLogDir;
        }

#if 0
        //! IsUploadLog IsUploadTestLog CustomLogDir
        //! 检查IsUploadLog是否是当前表列
        if (mapFiledsNameVal.end() == mapFiledsNameVal.find("IsUploadLog"))
        {
            sTaskSQL.msTaskConfig.mbIsUploadLog = false;
        }
        else
        {
            sTaskSQL.msTaskConfig.mbIsUploadLog = (1 == atoi(mapFiledsNameVal["IsUploadLog"].c_str()));
        }
        //! 检查IsUploadTestLog是否是当前表列
        if (mapFiledsNameVal.end() == mapFiledsNameVal.find("IsUploadTestLog"))
        {
            sTaskSQL.msTaskConfig.mbIsUploadTestLog = true;
        }
        else
        {
            sTaskSQL.msTaskConfig.mbIsUploadTestLog = (1 == atoi(mapFiledsNameVal["IsUploadTestLog"].c_str()));
        }
        //! 检查CustomLogDir是否是当前表列
        if (mapFiledsNameVal.end() == mapFiledsNameVal.find("CustomLogDir"))
        {
            sTaskSQL.msTaskConfig.mstrCustomLogDir = L"";;
        }
        else
        {
            sTaskSQL.msTaskConfig.mstrCustomLogDir = JGW_A2W(mapFiledsNameVal["CustomLogDir"],CP_UTF8);
        }
#endif
        return true;
    }
