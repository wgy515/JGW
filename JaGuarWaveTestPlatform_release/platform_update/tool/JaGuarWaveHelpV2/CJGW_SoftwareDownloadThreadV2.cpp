...
namespace JGW
{
    CCJGW_SoftwareDownloadThreadV2::CCJGW_SoftwareDownloadThreadV2(S_JGW_HELP_V2_CONFIG& sHelpV2Config) : msHelpV2Config(sHelpV2Config),mpHelpServiceThread(NULL),mbInstallUpdate(false),mcCurlFtpDownload(sHelpV2Config.mstrFTPHost,sHelpV2Config.mstrUserName,sHelpV2Config.mstrUserPassword,sHelpV2Config.mnFTPPort)
    {
    }
    ...
    {
        bool bUpdate = false;
        mpHelpServiceThread = (CCJGW_UpdateServiceThread*)msHelpV2Config.msDownloadThreadParam.mpParam;

        mcCurlFtpDownload.UpdateHostUserNamePassword(msHelpV2Config.mstrFTPHost,msHelpV2Config.mstrUserName,msHelpV2Config.mstrUserPassword,msHelpV2Config.mnFTPPort);
        if (!LoadHelpDB())
        {
            msHelpV2Config.msDownloadThreadParam.mstrJsonCommand = "{\"helpRspType\":-3,\"status\":\"false\",\"bUpdate\":\"false\",\"error_msg\":\"Failed to download or parse the update database\"}";
            mpHelpServiceThread->OnRspDownloadThreadResultMsg(WPARAM(&msHelpV2Config.msDownloadThreadParam),NULL);
            goto CHECK_END;
        }

        if (!CheckInstallVersion(bUpdate))
        {
            msHelpV2Config.msDownloadThreadParam.mstrJsonCommand = "{\"helpRspType\":-3,\"status\":\"false\",\"bUpdate\":\"false\",\"error_msg\":\"Software installation failed to download\"}";
            mpHelpServiceThread->OnRspDownloadThreadResultMsg(WPARAM(&msHelpV2Config.msDownloadThreadParam),NULL);
            goto CHECK_END;
        }
        ...
        {
            msHelpV2Config.msDownloadThreadParam.mstrJsonCommand = "{\"helpRspType\":-3,\"status\":\"false\",\"bUpdate\":\"false\",\"error_msg\":\"OTA update pack download failed\"}";
            mpHelpServiceThread->OnRspDownloadThreadResultMsg(WPARAM(&msHelpV2Config.msDownloadThreadParam),NULL);
            goto CHECK_END;
        }
        ...
        {
            msHelpV2Config.msDownloadThreadParam.mstrJsonCommand = "{\"helpRspType\":-3,\"status\":\"false\",\"bUpdate\":\"false\",\"error_msg\":\"Configuration update package download failed\"}";
            mpHelpServiceThread->OnRspDownloadThreadResultMsg(WPARAM(&msHelpV2Config.msDownloadThreadParam),NULL);
            goto CHECK_END;
        }
...
CHECK_END:
    ...
    bool CCJGW_SoftwareDownloadThreadV2::LoadHelpDB()
    {
        ...
        long lFtpFileTime = 0;
        JGW_FormatWString(strHelpDBFTPPath,L"%s/help.db",msHelpV2Config.mstrFTPRootPath.c_str());
        JGW_FormatWString(strDownloadDBFTPPath,L"%s\\temp.db",JGW_GetApplicationFolder());
        //! 是否已经下载加载过DB文件
        //! 2020-10-00 14:22:35
        if (msHelpV2Config.mbIsDownloadDB && msHelpV2Config.mlHelpDBFtpUpdateTime >= 1601446955 && !msHelpV2Config.msDownloadThreadParam.mbIsUpdateTaskNumber)
        {
            if (mcCurlFtpDownload.GetCurlFtpFileTimeAndFileSize(strHelpDBFTPPath,strErrorMsg,lFtpFileTime,dFileSize) && lFtpFileTime == msHelpV2Config.mlHelpDBFtpUpdateTime)
            {
        ...
        if (!helpSqliteDB.LoadHelpSqlite3(JGW_W2A(strDownloadDBFTPPath),msHelpV2Config.msDownloadThreadParam.mstrTaskNumber,msHelpV2Config.msTaskSQL))
        {
        ...
        if (!mcCurlFtpDownload.GetCurlFtpFileTimeAndFileSize(strHelpDBFTPPath,strErrorMsg,msHelpV2Config.mlHelpDBFtpUpdateTime,dFileSize))
        {
            msHelpV2Config.mlHelpDBFtpUpdateTime = -1;
            return true;
        }
        msHelpV2Config.mbIsDownloadDB = true;
        return true;
    }
    ...
    bool CCJGW_SoftwareDownloadThreadV2::CheckInstallVersion(bool& bUpdate)
    {
        bUpdate = true;
        //! 检查软件版本
        if (0 == JGW_WStrComparenoCaseWStr(msHelpV2Config.msTaskSQL.msInstallVersion.mstrVersion.c_str(),msHelpV2Config.msVersionConfig.mstrInstallVersion.c_str()))
        {
            bUpdate = false;
        ...
        do 
        {
            ...
            if (JGW_FileExistsToFilePath(_T(DEFAULT_INSTALL_FILE_PATH)))
            {
                ...
                if (NULL == JGW_WStrComparenoCaseWStr(strMD5.c_str(),msHelpV2Config.msTaskSQL.msInstallVersion.mstrMD5.c_str()))
                {
            ...
            if (!mcCurlFtpDownload.DownloadCurlFtpFile(msHelpV2Config.msTaskSQL.msInstallVersion.mstrFTPUrl,_T(DEFAULT_INSTALL_FILE_PATH),strErrorMsg))
            {
                PrintLogW(L"FTP Download Install File(%s) Fail:%s",msHelpV2Config.msTaskSQL.msInstallVersion.mstrFTPUrl.c_str(),strErrorMsg.c_str());
                JGW_DeleteFile(_T(DEFAULT_INSTALL_FILE_PATH));
                return false;
            }
...
DOWNLOAD_END:
    ...
    bool CCJGW_SoftwareDownloadThreadV2::CheckOTAVersion(bool& bUpdate)
    {
        bUpdate = true;
        //! 表示没有OTA相关版本信息
        if (msHelpV2Config.msTaskSQL.mnOTAID <= 0)
        {
            bUpdate = false;
            return true;
        }
        ...
        //! 检查OTA版本
        if (0 == JGW_WStrComparenoCaseWStr(msHelpV2Config.msTaskSQL.msOTA.mstrVersion.c_str(),msHelpV2Config.msVersionConfig.mstrUpdateVersion.c_str()))
        {
            bUpdate = false;
        ...
        do 
        {
            ...
            if (JGW_FileExistsToFilePath(_T(DEFAULT_UPDATE_FILE_PATH)))
            {
                ...
                //! 如果已经下载则检查MD5是否一致如果一致直接发送更新命令即可
                if (NULL == JGW_WStrComparenoCaseWStr(strMD5.c_str(),msHelpV2Config.msTaskSQL.msOTA.mstrMD5.c_str()))
                {
                    //msHelpV2Config.msDownloadThreadParam.mstrJsonCommand = "{\"helpRspType\":1,\"status\":\"true\",\"bUpdate\":\"true\",\"error_msg\":\"\"}";
                    JGW_FormatString(msHelpV2Config.msDownloadThreadParam.mstrJsonCommand,"{\"helpRspType\":3,\"status\":\"true\",\"bUpdate\":\"true\",\"error_msg\":\"\",\"version\":\"%s\"}",JGW_W2A(msHelpV2Config.msTaskSQL.msOTA.mstrVersion).c_str());
                    goto DOWNLOAD_END;
                }
            ...
            std::wstring strErrorMsg;
            if (!mcCurlFtpDownload.DownloadCurlFtpFile(msHelpV2Config.msTaskSQL.msOTA.mstrFTPUrl,_T(DEFAULT_UPDATE_FILE_PATH),strErrorMsg))
            {
                PrintLogW(L"FTP Download OTA File(%s) Fail:%s",msHelpV2Config.msTaskSQL.msOTA.mstrFTPUrl.c_str(),strErrorMsg.c_str());
                JGW_DeleteFile(_T(DEFAULT_UPDATE_FILE_PATH));
                return false;
            }
        } while (nInstallDownload-- > 0);
...
DOWNLOAD_END:
    ...
    bool CCJGW_SoftwareDownloadThreadV2::CheckConfigVersion(bool& bUpdate)
    {
        bUpdate = true;
        //! 检查CONFIF版本
        if (0 == JGW_WStrComparenoCaseWStr(msHelpV2Config.msTaskSQL.msConfig.mstrVersion.c_str(),msHelpV2Config.msVersionConfig.mstrConfigVersion.c_str()))
        {
            bUpdate = false;
            JGW_FormatString(msHelpV2Config.msDownloadThreadParam.mstrJsonCommand,"{\"helpRspType\":3,\"status\":\"true\",\"bUpdate\":\"false\",\"error_msg\":\"\",\"version\":\"%s\"}",JGW_W2A(msHelpV2Config.msVersionConfig.mstrConfigVersion).c_str());
            //msHelpV2Config.msDownloadThreadParam.mstrJsonCommand = "{\"helpRspType\":1,\"status\":\"true\",\"bUpdate\":\"false\",\"error_msg\":\"\"}";
            goto DOWNLOAD_END;
        }
        ...
        do 
        {
            ...
            if (JGW_FileExistsToFilePath(_T(DEFAULT_UPDATE_FILE_PATH)))
            {
                std::wstring strMD5 = JGW_A2W(CCJGW_CryptFileMD5::GetFileMD5A(DEFAULT_UPDATE_FILE_PATH),CP_UTF8);
                //! 如果已经下载则检查MD5是否一致如果一致直接发送更新命令即可
                if (NULL == JGW_WStrComparenoCaseWStr(strMD5.c_str(),msHelpV2Config.msTaskSQL.msConfig.mstrMD5.c_str()))
                {
                    JGW_FormatString(msHelpV2Config.msDownloadThreadParam.mstrJsonCommand,"{\"helpRspType\":3,\"status\":\"true\",\"bUpdate\":\"true\",\"error_msg\":\"\",\"version\":\"%s\"}",JGW_W2A(msHelpV2Config.msTaskSQL.msConfig.mstrVersion).c_str());
                    //msHelpV2Config.msDownloadThreadParam.mstrJsonCommand = "{\"helpRspType\":1,\"status\":\"true\",\"bUpdate\":\"true\",\"error_msg\":\"\"}";
                    goto DOWNLOAD_END;
                }
                JGW_DeleteFile(_T(DEFAULT_UPDATE_FILE_PATH));
            ...
            std::wstring strErrorMsg;
            if (!mcCurlFtpDownload.DownloadCurlFtpFile(msHelpV2Config.msTaskSQL.msConfig.mstrFTPUrl,_T(DEFAULT_UPDATE_FILE_PATH),strErrorMsg))
            {
                PrintLogW(L"FTP Download Update Config File(%s) Fail:%s",msHelpV2Config.msTaskSQL.msConfig.mstrFTPUrl.c_str(),strErrorMsg.c_str());
                JGW_DeleteFile(_T(DEFAULT_UPDATE_FILE_PATH));
                return false;
            }
        } while (nInstallDownload-- > 0);
