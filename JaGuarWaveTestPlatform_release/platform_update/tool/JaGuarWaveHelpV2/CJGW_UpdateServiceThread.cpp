...
namespace JGW
{
    CCJGW_UpdateServiceThread::CCJGW_UpdateServiceThread(S_JGW_HELP_V2_CONFIG& sHelpV2Config) : msHelpV2Config(sHelpV2Config),macceptorSocket(mios,boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(),sHelpV2Config.mnUpdateServiceSocketPort)),/*mDownloadThread(sHelpConfig),*/mdeadline_(mios),mstrHeartbeatPacket("{\"helpRspType\":-1,\"status\":\"true\",\"bUpdate\":\"false\",\"error_msg\":\"\"}")
    {
        mTxBuf = (char*)calloc(0x01,SOCKET_TX_BUF_MAX);
        mRxBuf = (char*)calloc(0x01,SOCKET_RX_BUF_MAX); 
        Log4WI_F(L"Server Port:%d",sHelpV2Config.mnUpdateServiceSocketPort);
    }
    ...
    void CCJGW_UpdateServiceThread::CheckDeadline(const boost::system::error_code e)
    {
        ...
        if (mdeadline_.expires_at() <= boost::asio::deadline_timer::traits_type::now())  
        {
            ...
            msHelpV2Config.msDownloadThreadParam.mClientSock = NULL;
    ...
    void CCJGW_UpdateServiceThread::SendHeartbeatPacket(const boost::system::error_code e)
    {
        ...
        if (mdeadline_.expires_at() <= boost::asio::deadline_timer::traits_type::now())  
        {
            if (msHelpV2Config.msDownloadThreadParam.mClientSock && msHelpV2Config.msDownloadThreadParam.mClientSock->is_open())
            {
                mAsyncWndMessage->PutAsyncStringMessage(WM_HWND_RICHEDIT_APPEND_TEST_MSG,JGW_A2W(mstrHeartbeatPacket).c_str());   
                msHelpV2Config.msDownloadThreadParam.mClientSock->async_write_some(boost::asio::buffer(mstrHeartbeatPacket),boost::bind(&CCJGW_UpdateServiceThread::SendSocketData,this,_1,_2,msHelpV2Config.msDownloadThreadParam.mClientSock));
    ...
    {
        if (ec) return;
        msHelpV2Config.msDownloadThreadParam.mClientSock = sock;
    ...
    void CCJGW_UpdateServiceThread::HandleHelpDownloadReq(CCJGW_FTM_Json& ftmJson,boost::shared_ptr<boost::asio::ip::tcp::socket> sock)
    {
        ...
        msHelpV2Config.msDownloadThreadParam.mpParam = this;
        msHelpV2Config.msDownloadThreadParam.mstrExecFolder = JGW_A2W(helpReqDownloadJson.execFolder,CP_UTF8);
        std::wstring strTask = JGW_A2W(helpReqDownloadJson.taskNumber,CP_UTF8);
        if (0 == JGW_WStrComparenoCaseWStr(strTask.c_str(),msHelpV2Config.msDownloadThreadParam.mstrTaskNumber.c_str()))
        {
            msHelpV2Config.msDownloadThreadParam.mbIsUpdateTaskNumber = false;
        }
        else
        {
            msHelpV2Config.msDownloadThreadParam.mbIsUpdateTaskNumber = true;
        }
        msHelpV2Config.msDownloadThreadParam.mstrTaskNumber = strTask;
    ...
    void CCJGW_UpdateServiceThread::LoadExecVersionConfig(const std::wstring& strExecFolder)
    {
        ...
        msHelpV2Config.msVersionConfig.mstrInstallVersion = versionIni.GetIniKeyValue(L"install",L"version");
        msHelpV2Config.msVersionConfig.mstrUpdateVersion = versionIni.GetIniKeyValue(L"update",L"version");
        msHelpV2Config.msVersionConfig.mstrConfigVersion = versionIni.GetIniKeyValue(L"config",L"version");
    }
}