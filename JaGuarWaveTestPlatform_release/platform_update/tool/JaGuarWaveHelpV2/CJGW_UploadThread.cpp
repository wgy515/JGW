...
namespace JGW
{
    CCJGW_UploadThread::CCJGW_UploadThread(S_JGW_HELP_V2_CONFIG& sHelpV2Config) : msHelpV2Config(sHelpV2Config),mstrTestPlatformExecFolder(L"C:\\JW\\exec\\")
    {
    ...
    {
        CCJGW_UpdateServiceThread* pThread = (CCJGW_UpdateServiceThread*)msHelpV2Config.msDownloadThreadParam.mpParam;
        CCJGW_FtpClient ftpClient;
        mstrTestPlatformExecFolder = msHelpV2Config.msDownloadThreadParam.mstrExecFolder;
#if 0
        if (mstrTestPlatformExecFolder.empty()) return ;
#else
        if (mstrTestPlatformExecFolder.empty())
        {
            mstrTestPlatformExecFolder = JGW_GetApplicationFolder();
            msHelpV2Config.msDownloadThreadParam.mstrExecFolder = mstrTestPlatformExecFolder;
        }
#endif
        ...
        if (!ftpClient.OpenFTPClient(msHelpV2Config.mstrFTPHost,msHelpV2Config.mstrUserName,msHelpV2Config.mstrUserPassword,msHelpV2Config.mnFTPPort))
        {
            msHelpV2Config.mnUploadIntervalTimeSec = 2 * 1000;
            goto UPLOAD_END;
        }
...
UPLOAD_END:
    ...
    {
        std::wstring strFTPUploadConfigPath(msHelpV2Config.mstrFTPLogRootPath);
    ...
    void CCJGW_UploadThread::LoadFTPUploadConfig(const std::wstring& strUploadConfigFilePath)
    {
        ...
        msHelpV2Config.mnUploadIntervalTimeSec = uploadConfigIni.GetIniKeyIntValue(L"Time",L"UploadIntervalTimeSec",5 * 60);
    ...
    void CCJGW_UploadThread::UploadPackageLogFileFTP(CCJGW_FtpClient& ftpClient,const std::wstring& strFolder)
    {
        ...
        //! 保存到本地目录
        JGW_FormatWString(strPackageLogFilePath,L"%s\\%s_update.zip",msHelpV2Config.msDownloadThreadParam.mstrExecFolder.c_str(),strUploadFolder.c_str());
         //! 检查打包上传的log文件是否存在，如果存在则直接上传，否则进行log打包
        //! 如果未查找到说明第一次开始上传，
        if (!JGW_FileExistsToFilePath(strPackageLogFilePath.c_str()))
        {
            //! 如果打包文件为空则说明打包失败，则直接返回
            if (!PackageLogFile(strFolder,strPackageLogFilePath))
            {
        ...
        //! 按照规则生成上传路径 订单-> 时间->TestLog_电脑名称.zip
        JGW_FormatWString(strUploadFolderFTP,L"%s/%s/%s/",msHelpV2Config.mstrFTPLogRootPath.c_str(),msHelpV2Config.msDownloadThreadParam.mstrTaskNumber.c_str(),JGW_GetTimeFolder().c_str());
        JGW_FormatWString(strUploadFilePathFTP,L"%s%s_%s_%d.zip",strUploadFolderFTP.c_str(),strUploadFolder.c_str(),JGW_A2W(GetHostNameIPAddr()).c_str(),JGW_GetTimeOfDay());
//         JGW_FormatWString(strUploadFilePathFTP,L"%s/%s/%s/%s_%s_%d.zip",msHelpV2Config.mstrFTPLogRootPath.c_str(),JGW_GetTimeFolder().c_str(),JGW_A2W(GetHostNameIPAddr()).c_str(),strUploadFolder.c_str(),msHelpV2Config.msDownloadThreadParam.mstrTaskNumber.c_str(),JGW_GetTimeOfDay());
//         JGW_FormatWString(strUploadFolderFTP,L"%s/%s/%s/",msHelpV2Config.mstrFTPLogRootPath.c_str(),JGW_GetTimeFolder().c_str(),JGW_A2W(GetHostNameIPAddr()).c_str());
        ...
        if (!ftpClient.UpdateFTPFile(strUploadFilePathFTP,strPackageLogFilePath))
        {
            ...
            {
                JGW_DeleteFile(strPackageLogFilePath);   
                PrintLogW(L"Update FTP Package Log File (%s => %s)",strPackageLogFilePath.c_str(),strUploadFilePathFTP.c_str());
            }
        }
        else
        {
    ...
    {
        //! 处理目录相关信息
        std::wstring strFolderPath = JGW_RealPathToAbsPathBasePath(strFolder.c_str(),msHelpV2Config.msDownloadThreadParam.mstrExecFolder.c_str()),strRealativeFolder(strFolder);
        strFolderPath = JGW_FolderReplaceWindowsFolderAddBackslash(strFolderPath);
        std::wstring strExecFolderPath (JGW_FolderReplaceWindowsFolderAddBackslash(msHelpV2Config.msDownloadThreadParam.mstrExecFolder));
