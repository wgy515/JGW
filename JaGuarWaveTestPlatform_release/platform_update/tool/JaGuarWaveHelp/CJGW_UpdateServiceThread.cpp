...
namespace JGW
{
    CCJGW_UpdateServiceThread::CCJGW_UpdateServiceThread(S_JGW_HELP_CONFIG& sHelpConfig) : msHelpConfig(sHelpConfig),macceptorSocket(mios,boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(),sHelpConfig.mnUpdateServiceSocketPort)),/*mDownloadThread(sHelpConfig),*/mdeadline_(mios),mstrHeartbeatPacket("{\"helpRspType\":-1,\"status\":\"true\",\"bUpdate\":\"false\",\"error_msg\":\"\"}")
    {
        mTxBuf = (char*)calloc(0x01,SOCKET_TX_BUF_MAX);
        mRxBuf = (char*)calloc(0x01,SOCKET_RX_BUF_MAX); 
        Log4WI_F(L"Server Port:%d",sHelpConfig.mnUpdateServiceSocketPort);
    }
    ...
    void CCJGW_UpdateServiceThread::CheckDeadline(const boost::system::error_code e)
    {
        ...
        if (mdeadline_.expires_at() <= boost::asio::deadline_timer::traits_type::now())  
        {
            ...
            msHelpConfig.msDownloadThreadParam.mClientSock = NULL;
    ...
    void CCJGW_UpdateServiceThread::SendHeartbeatPacket(const boost::system::error_code e)
    {
        ...
        if (mdeadline_.expires_at() <= boost::asio::deadline_timer::traits_type::now())  
        {
            if (msHelpConfig.msDownloadThreadParam.mClientSock && msHelpConfig.msDownloadThreadParam.mClientSock->is_open())
            {
                mAsyncWndMessage->PutAsyncStringMessage(WM_HWND_RICHEDIT_APPEND_TEST_MSG,JGW_A2W(mstrHeartbeatPacket).c_str());   
                msHelpConfig.msDownloadThreadParam.mClientSock->async_write_some(boost::asio::buffer(mstrHeartbeatPacket),boost::bind(&CCJGW_UpdateServiceThread::SendSocketData,this,_1,_2,msHelpConfig.msDownloadThreadParam.mClientSock));
    ...
    {
        LPS_JGW_HELP_DOWNLOAD_THREAD_PARAM psDownloadThreadParam = (LPS_JGW_HELP_DOWNLOAD_THREAD_PARAM)wParam;
    ...
    void CCJGW_UpdateServiceThread::SendSocketData(const boost::system::error_code& ec,size_t txLenCompleted,boost::shared_ptr<boost::asio::ip::tcp::socket> sock)
    {
     ...
     void CCJGW_UpdateServiceThread::HandleHelpDownloadReq(CCJGW_FTM_Json& ftmJson,boost::shared_ptr<boost::asio::ip::tcp::socket> sock)
     {
         ...
         //! 
         //! msHelpConfig.msDownloadThreadParam.mClientSock = sock;
         msHelpConfig.msDownloadThreadParam.mpParam = this;
         msHelpConfig.msDownloadThreadParam.mstrExecFolder = JGW_A2W(helpReqDownloadJson.execFolder,CP_UTF8);
         msHelpConfig.msDownloadThreadParam.mstrTaskNumber = JGW_A2W(helpReqDownloadJson.taskNumber,CP_UTF8);
     ...
     void CCJGW_UpdateServiceThread::LoadExecVersionConfig(const std::wstring& strExecFolder)
     {
         ...
         msHelpConfig.msVersionConfig.mstrInstallVersion = versionIni.GetIniKeyValue(L"install",L"version");
         msHelpConfig.msVersionConfig.mstrUpdateVersion = versionIni.GetIniKeyValue(L"update",L"version");
     }
}