...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
  ...
  <xsl:key name="dscNameDistinct" match="DataSetCollection" use="Name"/>
...
<!-- Notice, I'm not copying the index attributes, since we want the next pass to be basically index-less -->
          ...
          <!-- 
          This case executes to process all unnamed datasetcollections.  If there are unnamed DSCs in many tests, we assume thes should all be grouped.
          Basically, we consider Name=NULL to be a distinct name.
          -->
          <xsl:if test="$alltests[@I=$currentInd]/DataSetCollection[not(Name)]">
            <DataSetCollection>
              <xsl:copy-of select="@*"/>
              <xsl:apply-templates select="$alltests[@I=$currentInd]/DataSetCollection[not(Name)]/DataSet"/>
            </DataSetCollection>
          </xsl:if>
          
          <!-- 
          This loop executes once per unique DSC name value, and only pays attention to the first test for each index
          If we need to act on all tests of this index, we'll use $alltests[@I=$currentInd]
          -->
          <xsl:for-each select="$alltests[@I=$currentInd]/DataSetCollection[generate-id() = generate-id(key('dscNameDistinct', Name)[1])]">
            <xsl:variable name="dscName" select="Name"/>
            <DataSetCollection>
              
              <xsl:copy-of select="@*"/>
              <xsl:copy-of select="Name"/>
              <xsl:apply-templates select="$alltests[@I=$currentInd]/DataSetCollection[Name=$dscName]/DataSet"/>
            </DataSetCollection>
