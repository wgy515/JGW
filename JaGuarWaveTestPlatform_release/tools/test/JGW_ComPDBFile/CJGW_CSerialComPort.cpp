...
//     }
    ...
    int CCJGW_CSerialComPort::ReadAsyncCommand(char* strRead,size_t len,long millis /* = 0 */)
    {
        ...
        while (tm.elapsed() <= fMillis)
        {
            ...
            if (bReader)
            {
                if (WAIT_OBJECT_0 != mSerialPort.GetOverlappedResultPortEx(&mov,dwBytesRead,(fMillis - tm.elapsed()) * 1000.00))
                {
        ...
        Log4AD_F("ReadATAsyncModem:%s",strRead);
    ...
    int CCJGW_CSerialComPort::WriteAsyncCommand(const char* szATCommand,size_t len,long millis /* = 0 */)
    {
        ...
        if (bWrite)
        {
            ...
            {
                Log4WI_F(L"WriteAsyncATModem GetOverlappedResultPortEx 失败 ERROR:%s" ,JGW_GetSystemErrorString(GetLastError()).c_str());
                return -1;
            }  
        }
    ...
    bool CCJGW_CSerialComPort::OpenPort(const wchar_t* szComName)
    {
        ...
        Log4WI_F(L"CCJGW_CSerialComPort::OpenPort Port:%s",strComName.c_str());
