...
namespace JGW
{
    ...
    {
        mpSWlanConfig->msMonitorInfo.m_cNIGPIBType = NI_GPIB_TYPE;
        mpSWlanConfig->msMonitorInfo.m_cInterfaceType = 0;
        mpSWlanConfig->msMonitorInfo.m_nGpibInterface = 0;
        mpSWlanConfig->msMonitorInfo.m_nPrimaryAddress = 20;
        mpSWlanConfig->msMonitorInfo.m_nSecAddress = 96;
        mpSWlanConfig->msMonitorInfo.m_strModelName = L"CMW500";
        //! Tx 
        mvstrTxLoss.push_back("1.0");
        mvstrTxLoss.push_back("1.0");
        mvstrTxLoss.push_back("1.0");
        mvstrTxLoss.push_back("1.0");
        //! rx
        mvstrRxLoss.push_back("1.0");
        mvstrRxLoss.push_back("1.0");
        mvstrRxLoss.push_back("1.0");
        mvstrRxLoss.push_back("1.0");
    }
    ...
    bool JGWWiFiTestConfig::ParesWiFiTestConfigXml(const TiXmlElement* xmlRoot)
    {
        //! Instrument
        {
            const TiXmlElement*	pInstrumentNode,*pIGPIBNode = NULL;
            std::string strTemp;
            std::wstring strModelName;
            pInstrumentNode = xmlRoot->FirstChildElement("Instrument");
            IsNull(pInstrumentNode,false);

            GetElementTextValueA(pInstrumentNode,"PartialClassName",strTemp,"NIGPIB");
            mpSWlanConfig->msMonitorInfo.m_cNIGPIBType = 0;
            if (0 != strTemp.compare("NIGPIB")) mpSWlanConfig->msMonitorInfo.m_cNIGPIBType = 1;
            ...
            IsNull(pIGPIBNode,false);

            GetElementTextValueA(pIGPIBNode,"BoardID",strTemp,"0");
            mpSWlanConfig->msMonitorInfo.m_nGpibInterface = atoi(strTemp.c_str());

            GetElementTextValueA(pIGPIBNode,"PrimAddress",strTemp,"20");
            mpSWlanConfig->msMonitorInfo.m_nPrimaryAddress = atoi(strTemp.c_str());

            GetElementTextValueA(pIGPIBNode,"ScndAddress",strTemp,"0");
            mpSWlanConfig->msMonitorInfo.m_nSecAddress = atoi(strTemp.c_str());
        }
        //! Loss
        {
            const TiXmlElement*	pLossNode = NULL,*pTxNode = NULL,*pRxNode = NULL;
            std::map<std::string,std::string> mtxMap,mrxMap;
            ...
            while ( pTxNode )
            {
                GetElementAttributeValueA(pTxNode,"Name",strName,"B");
                GetElementAttributeValueA(pTxNode,"Value",strTemp,"B");
            ...
            while ( pRxNode )
            {
                GetElementAttributeValueA(pRxNode,"Name",strName,"B");
                GetElementAttributeValueA(pRxNode,"Value",strTemp,"1");
                mrxMap[strName] = strTemp;
                pRxNode = pRxNode->NextSiblingElement();
            }

            mvstrTxLoss.push_back(mtxMap["B"]);
            mvstrTxLoss.push_back(mtxMap["G"]);
            mvstrTxLoss.push_back(mtxMap["N"]);
            mvstrTxLoss.push_back(mtxMap["AC"]);

            mvstrRxLoss.push_back(mrxMap["B"]);
            mvstrRxLoss.push_back(mrxMap["G"]);
            mvstrRxLoss.push_back(mrxMap["N"]);
            mvstrRxLoss.push_back(mrxMap["AC"]);
        }
        return true;
    }
    ...
    void JGWWiFiTestConfig::BuildLoss(size_t nTxChain,size_t channelSize,std::vector<std::vector<float>>& vvLoss,std::vector<std::string>& vstrLoss)
    {
        for (size_t i = 0;i < vstrLoss.size();i ++)
        {
            ...
            JGW_ParserFloatA(vstrTemp.at(nTxChain - 1).c_str(),",",vfLoss);
    ...
    bool JGWWiFiTestConfig::PareseTestNodeToCacheXML(const TiXmlElement* xmlTestNode)
    {
        ...
        vstrValue.clear();
        JGW_ParserFloatW(strValue.c_str(),L",",sWlanModeConfig.mvChannel);
        ...
        BuildLoss(sWlanModeConfig.mnTxChain,sWlanModeConfig.mvChannel.size(),vvRxLoss,mvstrRxLoss);
        BuildLoss(sWlanModeConfig.mnTxChain,sWlanModeConfig.mvChannel.size(),vvTxLoss,mvstrTxLoss);

        strValue = vInfoItem[L"Rx"].mstrValue;
        BuldInfoItemParam(1,sWlanModeConfig.mvWlanRxDataRate.size(),sWlanModeConfig.mvChannel.size(),vvRxTest,strValue);

        strValue = vInfoItem[L"Tx"].mstrValue;
        BuldInfoItemParam(1,sWlanModeConfig.mvWlanRxDataRate.size(),sWlanModeConfig.mvChannel.size(),vvTxTest,strValue);

        strValue = vInfoItem[L"ExpectedPower"].mstrValue;
        BuldInfoItemParam(1,sWlanModeConfig.mvWlanRxDataRate.size(),sWlanModeConfig.mvChannel.size(),vvfExpectPower,strValue);

        strValue = vInfoItem[L"Power"].mstrValue;
        BuldInfoItemParam(1,sWlanModeConfig.mvWlanRxDataRate.size(),sWlanModeConfig.mvChannel.size(),vvfPower,strValue);

        strValue = vInfoItem[L"PER"].mstrValue;
        BuldInfoItemParam(1,sWlanModeConfig.mvWlanRxDataRate.size(),sWlanModeConfig.mvChannel.size(),vvfPER,strValue);

        strValue = vInfoItem[L"RSSI"].mstrValue;
        BuldInfoItemParam(1,sWlanModeConfig.mvWlanRxDataRate.size(),sWlanModeConfig.mvChannel.size(),vvfRssi,strValue);

        strValue = vInfoItem[L"Power"].mstrRange;
        BuldInfoItemParam(1,sWlanModeConfig.mvWlanRxDataRate.size(),sWlanModeConfig.mvChannel.size(),vvfPowerRange,strValue);

        strValue = vInfoItem[L"RSSI"].mstrRange;
        BuldInfoItemParam(1,sWlanModeConfig.mvWlanRxDataRate.size(),sWlanModeConfig.mvChannel.size(),vvfRssiRange,strValue);


        for (size_t y = 0;y < sWlanModeConfig.mvWlanRxDataRate.size();y ++)
        {
            for (size_t z = 0;z < sWlanModeConfig.mvChannel.size();z ++)
            {
