...
#define	GYRO_ADDRESS (MPU9250_ADDRESS << 1)	  //陀螺地址 0xD0
#define MAG_ADDRESS (AK8963_ADDRESS << 1) //! 0x18
...
{
    float tepspps = sqrt(3.0f / 4.0f) * PI_3;
    float hhhhh = sqrt(3.0f / 4.0f) * DEGTORAD(1);
    float ax = -0.00482f;
    float ay = 0.0055;
    float az = 1.00732;
    float gx = -0.09;
    ...
    int index = 10;
#if 0
    char index = GYRO_ADDRESS;
    index = MAG_ADDRESS;
    //^0,*
    std::wstring s = L"0,2.266666,1,0.";
    std::wstring strRegex = L"^1,.*?";
    std::wregex partten(strRegex,std::wregex::icase);;


    if (regex_match (s,partten))
    {
    ...
    std::string szCmd = "adb shell";//cmd执行成功返回S_OK,失败返回E_FAIL  192.168.1.1为服务器地址
