...
/*        void read_sahara_download_to_port(const boost::system::error_code& ec,size_t rx_len);*/
            ...
            BOOST_ASIO_MOVE_ARG(WriteHandler) handler)
        {
            ...
            mdeadline_.async_wait(boost::bind(&CCJGW_DownloadPumaAsioSerialPort::CheckRWDeadline, this,_1));
            mnRecvIndex = 0;
            dbg_debug(LOG_DEBUG,msDownloadProgressInfo.mnIndex,strCmd);
    ...
    private:
        ...
        //! 当前下载错误消息
        void SendDownloadErrorMsg(const char* msg,E_DOWNLOAD_STATUS eDownloadStatus = E_ERROR_DOWNLOAD);
        //! 当前下载OK消息
        void SendDownloadOkMsg(const char* msg,E_DOWNLOAD_STATUS eDownloadStatus = E_SUCCESSFUL_DOWNLOAD);
    private:
        //! 下载步骤
