...
namespace JGW
{
    ...
    void CAdapterCommon::GetAdapterInfo(std::vector<NetWorkConection>& adapters)
    {
        ...
        if (ERROR_SUCCESS == nRel)
        {
            while (pIpAdapterInfo)
            {
                ...
                connection.index = pIpAdapterInfo->Index;
                connection.description = pIpAdapterInfo->Description;
                connection.ip_address = JGW_A2W_A(pIpAdapterInfo->IpAddressList.IpAddress.String);
                connection.subnet_mask = JGW_A2W_A(pIpAdapterInfo->IpAddressList.IpMask.String);
                connection.default_gateway = JGW_A2W_A(pIpAdapterInfo->GatewayList.IpAddress.String);
    ...
    void CAdapterCommon::GetIfTableInfo(std::vector<NetWorkConection>& adapters, MIB_IFTABLE* pIfTable)
    {
    ...
    int CAdapterCommon::FindConnectionInAdapterList(std::string connection, const std::vector<NetWorkConection>& adapters)
    {
