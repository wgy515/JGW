    ...
    int       nCmdShow)
{
    ...
    LoadString(hInstance, IDC_CEF3MESSAGELOOPDEMO, szWindowClass, MAX_LOADSTRING);
    ...
    hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_CEF3MESSAGELOOPDEMO));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_CEF3MESSAGELOOPDEMO));
    wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
    wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_CEF3MESSAGELOOPDEMO);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    InitializeCef3Processes(hInstance,true,strSubProcessesPath.c_str());
    ...
    CreateBrowserHelp(hWnd,rect,L"https://zipres.com/prs_connect_status.html");
    //CreateBrowserHelp(hWnd,rect,L"file:///G:/CEF/cef_binary_83.5.0+gbf03589+chromium-83.0.4103.106_windows32/tests/cefclient/resources/dialogs.html");
#endif
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CLOSE:
#if 1
        if (!IsCloseCef3Browser())
        {
            CloseAllBrowsers(false);

            // 第二步：等待OnBeforeClose触发
            //while (!IsCloseCef3Browser()) {
                //CefDoMessageLoopWork();  // 处理残留消息:ml-citation{ref="3" data="citationList"}
