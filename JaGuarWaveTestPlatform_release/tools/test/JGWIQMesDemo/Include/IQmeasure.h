 ...
 *   - LP_IQLINK_SetUdp();
 *   - LP_IQLINK_SetAp();
 *   - LP_IQLINK_Uplink_Throughput_Start();
 *   - LP_IQLINK_Downlink_Throughput_Start();
 *   - LP_IQLINK_PER_Prepare();
 *   - LP_IQLINK_PER_Start();
 *   - LP_IQLINK_PER_Stop();
 ...
 *
 * \subsection revision_3_1_0 3.1.0
 * Date: March 30, 2015
 * -# IQxel  IQapis Version:2.0.14.1 
...
enum LP_TESTER_CONTROL
{
	...
	TESTER_CONTROL_SCPI_11AD = 2,                             /*!< Using SCPI to control LP tester  */
...
enum SIGNAL_MODE
{
	...
	UP_TO_2GHz_SIGNAL = UP_TO_80MHZ_SIGNAL
};
 ...
 *		- 7 = VHT160 wideband capture
 * \param[in] sampleFreqHz IQxel options: 80Mhz/160Mhz;IQ2010/IQnxn/IQview 80Mhz only (Forced to set as 80Mhz if set different from 80Mhz)
 * \param[optional] segmentMode specifies if the capture should overwrite previous capture, or append to it. Valid options are:
  *		- 0 = overwrite previous capture, default
 *		- 1 = append to previous capture
 * \return ERR_OK if the data capture is successful; otherwise call LP_GetErrorString() for detailed error message.
 * \remark modeHT40 only needs to set to 1 if the HT40 mask (120MHz) analysis is desired.  For LP_Analyze80211n(), this flag can be set to 0.
 */
IQMEASURE_API int		LP_VsaCapture(double samplingTimeSecs, int triggerType=6, double sampleFreqHz=160e6,
									int ht40Mode=OFF, IQMEASURE_CAPTURE_NONBLOCKING_STATES nonBlockingState=BLOCKING, SEGMENT_CAPTURE_MODE segmentMode=SINGLE);
...
IQMEASURE_API int LP_Segms_Analyze80211ad(int numSegmsToAnalyze, char *mode ="nxn",
                                          int enablePhaseCorr            = 1,
                                          int enableSymTimingCorr        = 1,
                                          int enableAmplitudeTracking    = 0,
                                          int decodePSDU                 = 0,
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Segms_AnalyzeCW(int numSegms, double intervalLength = 0.0, double measurementLength = 0.0);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_SetTcp(int bufferLength, int tcpWindowSize, int maxSegmSize, int port = 5001);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_SetUdp(int udpBandwidth, int udpBufferSize, int udpPaketSize, int port = 5001);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_Rx_Per_Init(IQLINK_DGEN_MODE dgenMode=LINK_IPERF3);

//! IQlink PER Begin
/*!
 * \param[in] outputPower, output power for PER test
 * \param[in] perPacketCount, number of packets for PER test
 * \param[in] perPsduLength, length in Bytes per packet
 * \param[in] triggerLevelRf1, RF1 trigger level for ACK
 * \param[in] triggerLevelRf2, RF2 trigger level for ACK
 * \param[in] triggerLevelRf3, RF3 trigger level for ACK
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_Rx_Per_Start(double outputPower, int perPacketCount, int perPsduLength, double triggerLevelRf1=-15, double triggerLevelRf2=-15, double triggerLevelRf3=-15);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_AutoPLCalibration(int port[], int freqBandSelect, double lowBandFreqSetting[3], double highBandFreqSetting[3], double lowBandpathLoss[][MAX_BUFFER_LEN], double highBandpathLoss[][MAX_BUFFER_LEN]);
...
static const int INVALID_CONFIG_VALUE = -999;
static const int MAX_NUMBER_STREAMS = 8;
static const int MAX_CHAR_BUFFER_SIZE = 1024;

/*! \brief Instrument identification
*/
struct InstrumentId {
    char manufacturer[MAX_CHAR_BUFFER_SIZE];
    char model[MAX_CHAR_BUFFER_SIZE];
    char serialNo[MAX_CHAR_BUFFER_SIZE];
    char firmwareRev[MAX_CHAR_BUFFER_SIZE];
};

/*! \brief Instrument hardware specs
*/
struct InstrumentSpecs {
    unsigned int rfInputs;
    unsigned int rfOutputs;
    int rfOutputMinPower;
    int rfOutputMaxPower;
    int rfInputFloor;
    int rfInputCeiling;
};

/*! \brief
    All power results should be reported in dBm
*/
struct EvmResults {
    double avgEvm;
    double avgPower;
    double avgPreamblePower;
    double rampOn;
    double rampOff;
    double iqImbalanceAmplitude;
...
/*! \brief
*/
struct PowerCwResults {
    double power;
};

/*! \brief
*/
struct ObtAnalyzerConfig {
    int streamOrder[MAX_NUMBER_STREAMS];
    BoolAuto phaseCorrection;
    BoolAuto symbolTimingCorrection;
    BoolAuto amplitudeTracking;
    BoolAuto decodePsdu;
    BoolAuto fullPacketChannelEstimation;
    BoolAuto frequencyCorrection;
    PpduFormat packetFormat;
    double preamblePowerStartSec;
    double preamblePowerStopSec;
};

/*! \brief
*/
struct ObtCaptureConfig {
    double frequency;
    double packetCaptureTime;
    double preTriggerDelay;
    double referencePowerAmplitude;                         //!< If set to INVALID_CONFIG_VALUE will use auto reference level
    char saveCapturedSignalToFile[MAX_CHAR_BUFFER_SIZE];    //!< If empty string given, do not save captured signal
};

/*! \brief
*/
struct ObtGeneratorConfig {
    int streamOrder[MAX_NUMBER_STREAMS];
    double frequency;
    DataRate_11AD rate;
    BandwidthType bandwidthType;
    SignalType signal;                          //!< Modulated signal vs CW - Default: Modulated
    unsigned int numberOfPackets;               //!< If 0 and signal type is MODULATED, continuously send packets - Default: 0
    char modulationFile[MAX_CHAR_BUFFER_SIZE];  //!< If empty string given, do not save captured signal - Default: empty string
    double outputPower;                         //!< The output power of the generated signal at each port - Default: 0
};
...
/*! \brief Performs any initialization that is required by the one box tester.

    \param[in] address The address of this device.  The format is defined by the instrument vendor.
    \param[in] forceInitialization Do any configuration necessary to get device to known working state.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtInit(char address[MAX_CHAR_BUFFER_SIZE], bool forceInitialization = false);

IQMEASURE_API ObtRet obtSetWirelessTechnology(WirelessTechnology tech);
...
/*************************************/
/* START Information Query Functions */
/*************************************/

/*! \brief Returns the revision of the DLL being used.

    \param[out] revision A period separated DLL revision number
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtGetDllRevision(char revision[MAX_CHAR_BUFFER_SIZE]);

/*! \brief Returns the device identification (manufacturer, model, serialNo, firmwareRev) for this one box tester, and hardware specs if requested.

    \param[out] id The instrument identification for this one box tester.
    \param[out] spec The hardware specifications for this one box tester.  Can be NULL if no specs required.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtGetInstrId(InstrumentId* id, InstrumentSpecs* specs = NULL);

/*! \brief Get the error a string representation given the error code.

    \param[in] retCode The return code returned from a function.
    \praam[out] retStr The return code given in a human readable format.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtGetErrorStr(ObtRet retCode, char retStr[MAX_CHAR_BUFFER_SIZE]);

/***********************************/
/* END Information Query Functions */
/***********************************/

/*************************************/
/* START Analyzer Specific Functions */
/*************************************/

/*! \brief Initialize an analyzer configuration structure to default values.

    \param[in] config The configuration struct to initialize with default values.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerConfigInit(ObtAnalyzerConfig* config);

/*! \brief Sets the one box tester in a state ready to receive and analyze a signal.

    \param[in] config The configuration object for analyzing a signal.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerPrepare(const ObtAnalyzerConfig* config);

/*! \brief Initialize an analyzer capture configuration structure to default values.
    NOTE: If empty string given for saveCapturedSignalToFile, do not save captured signal.

    \param[in] config The configuration struct to initialize with default values.
    \param[in] tech The wireless technology for testing.
    \param[in] freq The center frequency of the signal.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerCaptureConfigInit(ObtCaptureConfig* config, double freq);

/*! \brief Start a data capture of the signal.

    \param[in] config The configuration options for this particular capture.
    \param[out] captureId The identification number of the capture taken when this function was called.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerCapture(const ObtCaptureConfig* config, unsigned int *captureId = NULL);

/*! \brief Analyze the EVM results of a capture.
           WARNING: Results alocation must be released with obtAnalyzerFreeResults(), otherwise a memory leak may occur.

    \param[out] results The results from the last capture or the capture given by captureId.
    \param[in] captureId The ID of the data to fetch.  If 0, the last capture will be used.
	\param[in] Number of captures to analyze. Analysis will occur on all captures simultaneously  Default is 1.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerGetEvmResults(EvmResults* results, unsigned int captureId = 1, int numberOfCaptures = 1);

/*! \brief Analyze the Spectral results of a capture.
           WARNING: Results alocation must be released with obtAnalyzerFreeResults(), otherwise a memory leak may occur.
    ...
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerGetSpectralMaskResults(SpectralMaskResults* results, unsigned int captureId = 1, int numberOfCaptures = 1);

/*! \brief Analyze the Power results of a capture.
           WARNING: Results alocation must be released with obtAnalyzerFreeResults(), otherwise a memory leak may occur.
    ...
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerGetPowerCwResults(PowerCwResults* results, unsigned int captureId = 1, int numberOfCaptures = 1);

/*! \brief Free results allocated by corresponding get call.

    \param[in] results The results to free.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerFreeEvmResults(EvmResults* results);
...
/*! \brief Free results allocated by corresponding get call.
    ...
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerFreeSpectralMaskResults(SpectralMaskResults* results);
...
/*! \brief Free results allocated by corresponding get call.
    ...
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtAnalyzerFreePowerCwResults(PowerCwResults* results);

/***********************************/
/* END Analyzer Specific Functions */
/***********************************/

/**************************************/
/* START Generator Specific Functions */
/**************************************/

/*! \brief Initialize a generator configuration structure to default values.

    \param[in] config The configuration struct to initialize with default values.
    \param[in] freq The center frequency of the signal.
    \param[in] powerLevel The output power of the signal.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtGeneratorConfigInit(ObtGeneratorConfig* config, double freq, double powerLevel);

/*! \brief Configure the one box tester to a state ready to generate a signal based on the given configuration.

    \param[in] config The configuration object for generating a signal.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtGeneratorPrepare(const ObtGeneratorConfig* config);

/*! \brief Start signal transmission.

    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtGeneratorStartTx();

/*! \brief End signal transmission.

    \param[in] timeout A timeout value in milliseconds to wait for the transmission to complete.
    \return A 0 value if the operation completed successfully.  Greater than 0 for a warning.  Less than 0 for a failure.
*/
IQMEASURE_API ObtRet obtGeneratorEndTx(unsigned int timeoutMs);

/************************************/
/* END Generator Specific Functions */
/************************************/
