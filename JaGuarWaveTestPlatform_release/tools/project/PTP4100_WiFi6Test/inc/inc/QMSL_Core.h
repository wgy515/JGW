/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Core/inc/QMSL_Core.h#26 $
 * $DateTime: 2022/09/28 20:26:03 $
 *
 *
 ******************************************************************************
 *
   ...
   */
   ...
   /**
      ...
      \param pMessageBuffer = pointer to a buffer of memory with the message.  This must be copied by the
                              user because after the call back is called, the buffer can be changed at
                        any time
      \param iJsonSize = size of json returned (in queryResultJson field) (0 if underlying transport mode is not QUTS)
	   \param pJsonBuffer = pointer to buffer with json message (NULL if underlying transport mode is not QUTS)
      \param hContextID = the same context ID used for function calls on a certain COM port.
   */

   typedef void( *asyncFQMessageCB )
      (
         unsigned short iMessageSize,
         unsigned char* iMessageBuffer,
         unsigned long  iJsonSize,
         unsigned char* pJsonBuffer,
         HANDLE hContextID
         );
...
*******************************************************************************/
   typedef long long QUTS_DEVICE_HANDLE_ID_TYPE;
   typedef long long QUTS_PROTOCOL_ID_TYPE;
   typedef unsigned int QUTS_PROTOCOL_TYPE;
   ...
   *******************************************************************************/
   ...
   that call back type.
      ...
      \param pEfsDirCallback   = Callback for the EfsDirectory() operation
      \param pGeneralSwDownloadCB = Callback for download events
      \param pAsyncFQMessageCB = Callback for filtering async messages (Binary and JSON String)

   \return None

   *******************************************************************************/
   QLIB_API void QLIB_ConfigureCallBacks_V2
   (
      HANDLE hResourceContext,
      EfsDirCB pEfsDirCallback,
      generalSWDownloadCB pGeneralSwDownloadCB,
      asyncFQMessageCB pAsyncFQMessageCB
   );
   ...
   *******************************************************************************/
   ...
   CDMA ICD, 2.1.2 Send generic diag command with delayed responses

   80-V1294-1. Command 127 (0x7E)
   ...
   directly.
      ...
      \param piResponseSize = Pointer to number of bytes received in the response packet
                        ...
                        value will be the actual number of bytes filled into the receive buffer.
      \param iResponseBytes = Pointer to a buffer to store the response packet contents.
      \param pDelayedResponseRecords = Pointer to a buffer to store the delayed response bye stream.  
									   Refer to DelayedResponseRecords in QLib_Defines.h
	  \param iTimeout = Number of milliseconds to wait for a timeout.
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_SendSyncWithDelayedResponses
   (
      HANDLE hResourceContext,
      short iRequestSize,
      unsigned char* piRequestBytes,
   ...
   *******************************************************************************/
   ...
   Get the library mode for the handle--QPST, QPHONEMS or QUTS mode.
   if handle NULL, return the default library mode.
   \param iLibraryMode = 1 QPST, 0: QPHONEMS, 2: QUTS
   \return void

   *******************************************************************************/

   QLIB_API unsigned char QLIB_GetLibraryMode(HANDLE hResourceContext, unsigned char* iLibraryMode);
   /******************************************************************************/
   /**
   ...
   *******************************************************************************/
   ...
   Get the target type of the QMSL handle.   See QLIB_TargetType_Enum in QLib_Defines.h
   for possible value

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()
   \param iTargetType = target type of the handle

   \return True if handle is valid.

   *******************************************************************************/
   QLIB_API unsigned char QLIB_GetTargetType(HANDLE hResourceContext, unsigned char * iTargetType);
      /******************************************************************************/
   /**
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_AnnotateDLF_Log_With_MPSS_Timestamp(
      HANDLE hResourceContext,
      char* sAnnotation
   );
   ...
   *******************************************************************************/
   ...
   Function to obfuscate QMSL hex command code for external version.

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()
   \param isObfuscation = If set true will obfuscate QMSL hex command.

   \return True if set success.


   *******************************************************************************/
   QLIB_API unsigned char QLIB_SetObfuscateFlag(HANDLE hResourceContext, bool isObfuscation);
   /******************************************************************************/
   /**
