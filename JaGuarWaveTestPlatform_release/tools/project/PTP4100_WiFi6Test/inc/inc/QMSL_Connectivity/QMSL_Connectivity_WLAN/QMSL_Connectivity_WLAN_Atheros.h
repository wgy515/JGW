/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Connectivity/inc/QMSL_Connectivity_WLAN/QMSL_Connectivity_WLAN_Atheros.h#46 $
 * $DateTime: 2022/08/25 23:26:06 $
 *
 *
 ******************************************************************************
 *
...
extern "C" {
   ...
   *******************************************************************************/
   ...
   typedef bool (*asyncGenMeasureCB)
	(unsigned int measureType, 
	 unsigned int freq, 
	 unsigned int bw,
	 int dataRate,
	 double powerLevel, 
   ...
   *******************************************************************************/
   ...
   QLIB_API char QLIB_FTM_WLAN_TLV2_Create16
   (
      HANDLE hResourceContext,
 ...
 /******************************************************************************/
 /**
   Atheros WLAN command to set QMSL timeout.
   ...
   \param   timeOut = QMSL timeout. Unit is ms.
               0: using default 4000 ms

   \return true if successful, false if failure

   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_SetTimeout
   (
	 HANDLE hResourceContext,
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_tlvReply
   (
     HANDLE hResourceContext,
     unsigned char *tlvStream, 
     int tlvSize, 
     unsigned char* reply, 
   ...
   *******************************************************************************/
   ...
   Atheros WLAN command to XTal patch clock setting

   Note: (Current supported chip set: QC6490)
   ...
   \warning QLIB_FTM_WLAN_SetModuleType() must be called first to set the WLAN module type
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_SetXtalCalOTPPatch
   (
   HANDLE hResourceContext
   );
   ...
   *******************************************************************************/
   ...
   unsigned int delayCapSettleMs,
   unsigned int delayPostXtalMs
   );

   /******************************************************************************/
   /**

   Atheros WLAN command to initialize xtal Calibration version 2

   Note: (Current supported chip set: IPQ807x)
       This cmd must be called after loading chip specified DLL 
         by calling QLIB_FTM_WLAN_Atheros_LoadDUT().
   ...
   \param iXtalTolerancePPM = the tolerance between the targeted xtal cal FreqErr and 
                              final calibrated xtal cal FreqErr in PPM.
   \param dXtalAlgorithm = 0: binary search. 1: linear search.
   \param dXtalStep = step of linear search.
   \param dXtalCalSaveOption = save to otp or not.
   \param delayCapSettleMs = delay settle time for DUT's CW tone.
   \param delayPostXtalMs = delay settle time after xtal calibration done.
   \param pmic = support XTAL calibration through PMIC .
   \return true if successful, false if failure
   \warning QLIB_FTM_WLAN_SetModuleType() must be called first to set the WLAN module type
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_xtal_CAL_INIT2
   (
   HANDLE hResourceContext,
   int iXtalTargetPPM,
   unsigned int iXtalTolerancePPM,
   unsigned int dXtalAlgorithm,
   unsigned int dXtalStep,
   unsigned int dXtalCalSaveOption,
   unsigned int delayCapSettleMs,
   unsigned int delayPostXtalMs,
   bool pmic
   ...
   *******************************************************************************/
   ...
   double _1PtRangeDbUL,
   double _1PtRangeDbLL,
   ...
   //LC
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_HWS_RD_Bench
   (
   HANDLE hResourceContext,
   unsigned int measureType,
   unsigned int freq,
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char  QLIB_FTM_WLAN_Atheros_Rx_FW_CAL2
   (
       HANDLE hResourceContext,
       asyncHandlerSG_CB pAsyncHandlerSigGenCB,
       asyncHandlerRxGainCalDoneCB pAsyncHandlerRxGainCalDoneCB,
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_TxSCPC_CAL_V1
   (
   HANDLE hResourceContext,
   asyncPMMessageCB pAsyncPMMessageCB,
   unsigned int measAvg
   ...
   *******************************************************************************/
   ...
   Note: This cmd works only when target is awake.
       (Current supported chip set: AR6006)
       This cmd must be called after calling QLIB_FTM_WLAN_Atheros_Tx_CALIBRATE_INIT().

   \return true if successful, false if failure
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_SELF_CAL_MEAS_V1
   (
   HANDLE hResourceContext,
   unsigned long iFreq, 
   unsigned long iTxChainMask,
...
//       when writing to OTP capability shall be allowed.
   ...
   *******************************************************************************/
   ...
   Atheros WLAN command to load or unload Wlan driver for WCN39xx and after.
   ...
   \warning QLIB_FTM_WLAN_SetModuleType() must be called first to set the WLAN module type
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_WLAN_Atheros_V2_Load_Driver
   ( 
   HANDLE hResourceContext,
   unsigned char iSubMode
   );
...
(
   ...
   Atheros WLAN program customer board id on EFUSE.

   \param 
         encryptFileName = encrypt file from CE team.

         offset =   efuse offset

         accountName = customer input account name or nickname

         boardID = 1 byte for Address #2

         status =  0: Write successfully and correctly
                   1 : buffer is out of bounds
                   2 : Write successfully but contents are incorrect

   \return true if successful, false if failure

   *******************************************************************************/
QLIB_API unsigned char QLIB_FTM_WLAN_SET_BOARD_ID_ACCOUNT
(
   HANDLE hResourceContext,
   char* encryptFileName,
   unsigned int offset,
   char* accountName, // customer input account name or nickname
   unsigned char boardID,  // 1 byte for Address #2   
   char* status      // FW returns status as 0, 1, 2. Tool defines status 3 as default for possible tlv2 error.
);
