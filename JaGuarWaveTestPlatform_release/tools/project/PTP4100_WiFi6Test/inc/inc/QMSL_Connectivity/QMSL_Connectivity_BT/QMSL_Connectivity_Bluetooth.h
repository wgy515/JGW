/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Connectivity/inc/QMSL_Connectivity_BT/QMSL_Connectivity_Bluetooth.h#72 $
 * $DateTime: 2022/09/29 15:06:42 $
 *
 *
 ******************************************************************************
 *
...
extern "C" {
  ...
  *******************************************************************************/
  ...
  \param  hResourceContext  = Resource context that was returned from the call to QLIB_ConnectServer()
  \param  Channel [in]      = Value for Channel: 0-78 used for BR/EDR, 0-39 used for LE/QHS
  ...
  *******************************************************************************/
  ...
  /******************************************************************************/
 /**
	  This method  Receive Bluetooth BR/EDR, LE or QHS packets in Burst or Continuous receive mode.
	  \param  hResourceContext 	= Resource context that was returned from the call to QLIB_ConnectServer()
	  \param  Channel [in]     	= Value for Channel , 0-78 used for BR/EDR, 0-39 used for LE/QHS
	  \param  iPacket_Type [in] 	= Value for Packet_Type
	  \code
	  ...
	  \endcode
	  \param iPayload_Pattern [in]  = Pattern to be used in test packets when Payload_Type = 0x08
	  \param iReceive_Type [in]    = Receive_Type
	  \code
	  0x0 = BURST_PACKETS,
	  0x1 = CONTINUOUS_MODE
	  \endcode
	  ...
	  \param iDeviceAddress [in]    = Value for BD_ADDR
	  \param iMax_Packets [in]      = Max Number of packets to transmit after TX mode is initiated
	  \param ioff_Slots [in] 		  = Number of off slots between RX packets. Note - field only for burst mode (not continuous)
	  \param iRx_SW_Gain [in] 	     = RX_SW_Gain parameter shall only be used when the Receive_Type is set to Continuous.
	  \code
	  0 = GS0 (Gain state 0), lowest gain,
	  1 = GS1(Gain state 1),
	  2 = GS2 (Gain state 2),
	  3 = GS3 (Gain state 3),
	  4 = GS4 (Gain state 4),
	  5 = GS5 (Gain state 5), highest gain
	  \encode
	  \return true if successful, false if fail.
	  *******************************************************************************/
	  QLIB_API unsigned char QLIB_FTM_BT_HCI_VS_PROD_TEST_RX_Enhanced(HANDLE hResourceContext, unsigned char iChannel, unsigned char iPacket_Type, unsigned short iPayload_Length, unsigned char iPayload_Type, unsigned long iPayload_Pattern, unsigned char iReceive_Type, unsigned char iLT_ADDR, unsigned char iHopping, unsigned char *iDeviceAddress, unsigned long iMax_Packets, unsigned char ioff_Slots, unsigned char iRx_SW_Gain);
	  /******************************************************************************/
  /**
	  This method retrieves accumulated test statistics on received packets and the number of errors in the packets received during the testing.
	  \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()
	  \param Packets_Received[out] = Packets_Received
	  \param Access_Code_Errors[out] = Access_Code_Errors
	  \param HEC_Errors[out] = HEC_Errors
	  \param CRC_Errors[out] = CRC_Errors
	  \param Total_Bit_Errors[out] = Total_Bit_Errors
	  \param RSSI[out] = RSSI
	  \return true if successful, false if fail.
	  \warning
	  *******************************************************************************/
	  QLIB_API unsigned char QLIB_FTM_BT_HCI_VS_PROD_TEST_RX_Stats_Enhanced(HANDLE hResourceContext, unsigned int* packetsReceived, unsigned int*  accessCodesErrors, unsigned int* hecErrors, unsigned int* crcErrors, unsigned int* totalBitErrors, int*  RSSI);
  ...
  *******************************************************************************/
   ...
   /******************************************************************************/
   /**
    This method is used to set the Board ID for given Account name

   \param hResourceContext [in] = Resource context that was returned from the call to QLIB_ConnectServer()
   \param encryptFileName[in] = encrypted file path.
   \param accountName[in] = pointer to 1 byte array holding the customer input account name.
   \param writeBoardID[in] = pointer to 1 byte array holding the board ID.
   \param readBoardID[out] = pointer to 2 byte array holding the board ID data written by this API for verification.

   \return true if successful, false if failure

   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_BT_SET_BOARD_ID_ACCOUNT
   (
	   HANDLE hResourceContext,
	   char* encryptFileName,
	   unsigned char* accountName, // customer input account name or nickname
	   unsigned char writeBoardID,
	   unsigned char* readBoardID
   );
   ...
   /******************************************************************************/
   /**
   This method is used to get the EDL Build info

   \param hResourceContext [in] = Resource context that was returned from the call to QLIB_ConnectServer()
   \param iedlInfoLength [out] = EDL Build Information Length
   \param oedlBuildInfo [out] = EDL Build Information

   \return true if successful, false if fail.
   \warning
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_BT_EDL_GET_BUILD_INFO(HANDLE hResourceContext, unsigned char *iedlInfoLength, unsigned char *oedlBuildInfo);
   ...
   *******************************************************************************/
   ...
   /******************************************************************************/
   /**
   This method is used to enable/disable the clock output

   \param hResourceContext [in] = Resource context that was returned from the call to QLIB_ConnectServer()
   \param ienable [in] = enable = 1 for enable the clock output, enable = 0 to disable
   \param iclockType [in] = 0x00 Low frequency clock (LFC) , 0x01 High frequency clock (HFC)

   \return true if successful, false if fail.
   \warning
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_BT_HCI_VS_MFG_Enable_CLK_Output(HANDLE hResourceContext, unsigned char enable, unsigned char clockType);
   /******************************************************************************/
   /**
   This method is used to update the coarse trim in real time for the specified clock.

   \param hResourceContext [in] = Resource context that was returned from the call to QLIB_ConnectServer()
   \param iclockType [in] = 0x00 Low frequency clock (LFC) , 0x01 High frequency clock (HFC)
   \param icoarse_trim [in] = Coarse trim value

   \return true if successful, false if fail.
   \warning
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_BT_HCI_VS_MFG_Update_CLK_Coarse_Trim(HANDLE hResourceContext, unsigned char clockType, unsigned char *coarse_trim);
   /******************************************************************************/
   /**
   This method is used to update the Fine trim in real time for the specified clock.

   \param hResourceContext [in] = Resource context that was returned from the call to QLIB_ConnectServer()
   \param iclockType [in] = 0x00 Low frequency clock (LFC) , 0x01 High frequency clock (HFC)
   \param ifine_Trim [in] = fine trim value

   \return true if successful, false if fail.
   \warning
   *******************************************************************************/
   QLIB_API unsigned char  QLIB_FTM_BT_HCI_VS_MFG_Update_CLK_Fine_Trim(HANDLE hResourceContext, unsigned char clockType, unsigned char *fine_Trim);

   /******************************************************************************/
   /**
   This method is used to store the specified clock trim persistently.
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char  QLIB_FTM_BT_HCI_VS_MFG_Update_Store_CLK_Trim(HANDLE hResourceContext, unsigned char clockType, unsigned char * coarse_trim, unsigned char * fine_Trim);
   /******************************************************************************* /
   /**
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_BT_NVM_DOWNLOAD(HANDLE hResourceContext,
      char *invmFileLocation , unsigned char  skipResponseChecks = 0);
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_BT_PATCH_DOWNLOAD(HANDLE hResourceContext,
      char *ipatchFileLocation , unsigned char  skipResponseChecks = 0);
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_BT_PROD_TEST_SUBCOMMAND_TEST_RX_CONTINUOUS(HANDLE hResourceContext, 
       unsigned char  iTechnologyType,
       unsigned char  ichannel,
       unsigned char  iSWGain
   );
   /******************************************************************************* /
    /**
