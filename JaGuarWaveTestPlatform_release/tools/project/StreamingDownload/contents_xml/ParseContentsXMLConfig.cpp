...
#define IsNull(x,result)\
	...
	void CParseContentsXMLConfig::PareseBuildNode( const TiXmlElement* pBuildNode )
	{
		...
		{
			std::wstring strFileName,strFilePath;
			pTempXmlNode = pBuildNode->FirstChildElement("download_file");
			while ( pTempXmlNode )
			{
				GetElementTextValue( pTempXmlNode,"file_name",strFileName );
				GetElementTextValue( pTempXmlNode,"file_path",strFilePath );
				mmapDownloadFiles.insert( std::map<std::string,std::string>::value_type( JGW_W2A(strFileName),JGW_W2A(strFilePath)) );
				pTempXmlNode = pTempXmlNode->NextSiblingElement();
			}
		}
