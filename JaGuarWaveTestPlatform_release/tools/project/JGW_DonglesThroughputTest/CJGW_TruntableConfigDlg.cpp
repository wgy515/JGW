...
namespace JGW
{
    CCJGW_TruntableSettingDlg::CCJGW_TruntableSettingDlg(S_ThroughputTest_Confige& sThroughputTestConfig,SCLLibHelper& cSCLLibHelper,HWND hParentHwnd /* = NULL */)
        : mhParentHwnd(hParentHwnd),mcSCLLibHelper(cSCLLibHelper),msThroughputTestConfig(sThroughputTestConfig),mnPosition(1)
    {
    }
    ...
    void CCJGW_TruntableSettingDlg::OnInitWindow()
    {
        ...
        {
            std::wstring strTemp;
            JGW_FormatWString(strTemp,L"%.0f",msThroughputTestConfig.mfVelocity);
            mpVelocityEdit->SetText(strTemp.c_str());
            JGW_FormatWString(strTemp,L"%.0f",msThroughputTestConfig.mfAcceleration);
            mpAccelerationEdit->SetText(strTemp.c_str());
            JGW_FormatWString(strTemp,L"%.0f",msThroughputTestConfig.mfDeceleration);
    ...
    int CCJGW_TruntableSettingDlg::DoModal()
    {
        ...
        CreateNoBorderStyleWindow(_T(""),_T("turntable_config.xml"),_T("Turntable Config"),mhParentHwnd,UI_WNDSTYLE_DIALOG);
#elif defined USE_ZIP_FILE_ENABLE
        SetResourceType(UILIB_ZIP,0,L"skin\\DonglesThroughputTest.zip");
        CreateNoBorderStyleWindow(_T(""),_T("turntable_config.xml"),_T("Turntable Config"),mhParentHwnd,UI_WNDSTYLE_DIALOG);
#else
        CreateNoBorderStyleWindow(_T("skin\\DonglesThroughputTest"),_T("turntable_config.xml"),_T("Turntable Config"),mhParentHwnd,UI_WNDSTYLE_DIALOG);
#endif
        CenterWindow();
    ...
    void CCJGW_TruntableSettingDlg::UpdateConfigParam()
    {
        ...
        msThroughputTestConfig.mfAcceleration = _ttof(mpAccelerationEdit->GetText().GetData());
        msThroughputTestConfig.mfDeceleration = _ttof(mpDecelerationEdit->GetText().GetData());
        msThroughputTestConfig.mfVelocity = _ttof(mpVelocityEdit->GetText().GetData());
    }
    ...
    void CCJGW_TruntableSettingDlg::OnStartPositionRotate()
    {
        ...
        //! 32
        mcSCLLibHelper.RelMove(TURNTABLE_NODE_ID,mnPosition * PER_ANGLE_TURN,&msThroughputTestConfig.mfVelocity,&msThroughputTestConfig.mfAcceleration,&msThroughputTestConfig.mfDeceleration);
