...
namespace JGW
{
    CCJGW_Iperf3Command::CCJGW_Iperf3Command(void) : mpIperf3TestInfoFunc(NULL),mbIperfRunning(false),mnSeekgLog(0),mbMultiThread(false),mnConnectedCount(0),mpContext(NULL),mpIperf3RawTestLogCallback(NULL),mpRawTestLogContext(NULL)
    {
    ...
    bool CCJGW_Iperf3Command::StartIper3ServerToUserParameters(const std::wstring& strUserParameters,const std::wstring& strIperf3Path /* = L"tooliperf3.exe" */)
    {
        std::string strIperApplicationPath;
        std::string strApplicationFolder;
        std::vector<std::string> vstrParameters;
        ...
        JGW_ParserStrA(LocalToUTF8(strUserParameters).c_str()," ",vstrParameters);
        for (std::vector<std::string>::iterator it = vstrParameters.begin();it != vstrParameters.end();++it)
        {
            if (std::string::npos == StrCaseStr<std::string>(it[0],"iperf"))
            {
                 mArgs.push_back(it[0]);
            }
        }
        mArgs.push_back("--logfile");
        mArgs.push_back(JGW_W2A(mstrIperfTestLogPath,CP_UTF8));
        mArgs.push_back("-i");
        mArgs.push_back("1");

        strApplicationFolder = JGW_W2A(JGW_GetApplicationFolder(),CP_UTF8);
        JGW_FormatString(strIperApplicationPath,"%s%s",strApplicationFolder.c_str(),JGW_W2A(strIperf3Path,CP_UTF8).c_str());
    ...
    void CCJGW_Iperf3Command::RegisterCallbackIperf3TestInfo(Iperf3TestInfoFunc func,void* pContext)
    {
        mpContext = pContext;
        mpIperf3TestInfoFunc = func;
    }

    bool CCJGW_Iperf3Command::StartIperf3ClientToUserParameters(const std::wstring& strUserParameters,const std::wstring& strIperf3Path /* = L"tooliperf3.exe" */)
    {
        std::string strIperApplicationPath;
        std::string strApplicationFolder;
        std::vector<std::string> vstrParameters;
    ...
    bool CCJGW_Iperf3Command::StartIperf3Client(const std::wstring& strIPAddress,size_t nTestTimeSec,size_t nThreadCount,const std::wstring& strExtParam /* = L"-w 8M -l 65000" */,const std::wstring& strIperf3Path /* = L"tooliperf3.exe" */)
    {
        ...
        mArgs.push_back("-c");
        mArgs.push_back(JGW_W2A(strIPAddress,CP_UTF8));
        mArgs.push_back("-t");
        ...
        {
            mArgs.push_back(JGW_W2A(it[0],CP_UTF8));
        }
        ...
        Sleep(250);
        mbIperfRunning = mRedirectConsole.LaunchConsole(strIperApplicationPath,mArgs,strApplicationFolder);
        Log4WI_F(L"Launch Console %s%s , result : %d",JGW_GetApplicationFolder(),strIperf3Path.c_str(),mbIperfRunning ? 1 : 0);
        if (mbIperfRunning)
        {
            CreateMessageThread(NULL,NULL,true);
        }
    ...
    bool CCJGW_Iperf3Command::GetIperf3Speed(float& fSpeed)
    {
        ...
        while (iFileStream.getline(szReadBuf,256))
        {
            ...
            std::wcout << szReadBuf << std::endl;
#endif
            if (mpIperf3RawTestLogCallback) mpIperf3RawTestLogCallback(szReadBuf,mpRawTestLogContext);
            ...
            if (mbMultiThread && NULL == wcsstr(szReadBuf,L"[SUM]")) continue;
            if (!HandIperfTestLog(szReadBuf,strID,start,end,fSpeed)) continue;
            //! size_t& start,size_t& end,float& speed
            if (mpIperf3TestInfoFunc) mpIperf3TestInfoFunc(fSpeed,mpContext);
            //if (1 == mnConnectedCount) mbMultiThread = false;
            mnConnectedCount = 0;
#endif
        }
    ...
    void CCJGW_Iperf3Command::RegisterCallbackIperf3RawTestLog(Iperf3RawTestLogCallback pIperf3RawTestLogCallback,void* pRawTestLogContext)
    {
        mpIperf3RawTestLogCallback = pIperf3RawTestLogCallback;
        mpRawTestLogContext = pRawTestLogContext;
    }
