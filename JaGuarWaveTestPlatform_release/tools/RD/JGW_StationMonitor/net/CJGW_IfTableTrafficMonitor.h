...
namespace JGW
{
    ...
    public:
        void SetNetCardConnectName(std::string& strConnectName);

        std::vector<NetWorkConection>& GetNetCardInfo();
        //! 获取网卡流量相关信息
        void GetNetCardFlowInfo(ULONGLONG& ulUploadSpeed,ULONGLONG& ulDownloadSpeed,float& txDropRate,float& rxDropRate,std::string& mac);
        //! 获取发送接收
       /* ULONGLONG GetTotalSentReceived();*/
        ...
        //!
        bool GetTxRxDropRatePhyAddress(ULONGLONG& ulUploadSpeed,ULONGLONG& ulDownloadSpeed,float& txDropRate,float& rxDropRate,std::string& mac);
    private:
