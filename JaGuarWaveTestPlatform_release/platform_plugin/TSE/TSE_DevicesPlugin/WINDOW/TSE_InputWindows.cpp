...
namespace JGW
{
    CTSE_InputWindows::CTSE_InputWindows(void) : mMainHwnd(NULL),mstrMaxChar(L"999"),mstrMinChar(L"0"),mbTSEInit(false),meStringUpperLowCaseType(E_STRING_UPPERCASE_TYPE)
    {
    }

    CTSE_InputWindows::~CTSE_InputWindows(void)
    {
    }

    const wchar_t* CTSE_InputWindows::TSE_GetParamDescription()
    {
        return L"{\
                \"RealName\": \"CTSE_InputWindows\",\
                \"TestName\": \"输入对话框\",\
                \"tip\": \"标题\",\
                \"input_length\": \"输入长度限制\",\
    ...
    bool CTSE_InputWindows::TSE_AddParam(const wchar_t* strParamName,const wchar_t* strParamValue)
    {
    ...
    bool CTSE_InputWindows::TSE_Init()
    {
        ...
        msAllInputUIAttrSetting.m_sWindowAttr.mstrWindowName = _T("CTSE_InputWindows_Duilib");
    ...
    bool CTSE_InputWindows::TSE_Run()
    {
		if (!mbTSEInit)
		{
			TSE_Init();
        ...
        return false;
#endif
        if ( IDOK == SendMessage(mMainHwnd,WM_HWND_INPUT_COMMON_WINDOW_MSG,WPARAM(&msAllInputUIAttrSetting),LPARAM(NULL)) && !msAllInputUIAttrSetting.m_sGetControlAttrSetting.mvsAttrNameValue[0].pstrValue.empty() )
        {
