#include "StdAfx.h"
#include "TSE_ReadWriteSerialComPort.h"
...
namespace JGW
{
    CTSE_ReadWriteSerialComPort::CTSE_ReadWriteSerialComPort(void) : mnCheckResultTimeOutSec(10),mnTestWaitResultMS(0),mnWriteTimeOutSec(10),mbIsHexReadWrite(false),mnReadResultRegexSearchIndex(0),mbReadOnly(false)
    {
    }


    CTSE_ReadWriteSerialComPort::~CTSE_ReadWriteSerialComPort(void)
    {
    }

    const wchar_t* CTSE_ReadWriteSerialComPort::TSE_GetParamDescription()
    {
        return L"{\
    ...
    bool CTSE_ReadWriteSerialComPort::TSE_AddParam(const wchar_t* strParamName,const wchar_t* strParamValue)
    {
    ...
    bool CTSE_ReadWriteSerialComPort::TSE_Init()
    {
	...
	bool CTSE_ReadWriteSerialComPort::WriteSerialBuffer()
	{
	...
	bool CTSE_ReadWriteSerialComPort::ReadSerialBuffer()
	{
		//! 是否保存读取结果，或者进行比对
		...
		while (jt.elapsed() <= mnCheckResultTimeOutSec)
		{
			...
			int len = CTSE_DevicesGlobalResource::GetSerialComPort().ReadAsyncCommand(szBuf,4096,mnCheckResultTimeOutSec * 1000);
			if (len < 0 )
			{
				return false;
			}
			...
			if (mbIsHexReadWrite)
			{
				for (int i = 0;i < len;i ++)
				{
					if (strReadBuf.empty() && strTemp.empty())
					{
			...
			strReadBuf += strTemp;
			LogI_F(L"Serial Read : %s",strTemp.c_str());
			//! 检查结果
    ...
    bool CTSE_ReadWriteSerialComPort::TSE_Run()
    {
