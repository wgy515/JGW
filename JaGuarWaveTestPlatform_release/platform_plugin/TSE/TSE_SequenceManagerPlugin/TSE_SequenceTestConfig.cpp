...
namespace JGW
{
    CTSE_SequenceTestConfig::CTSE_SequenceTestConfig(S_TSE_SequenceConfig& suitetestconfig,CTSE_LogServices& logServices):m_sSuiteTestConfig(suitetestconfig),mLogServices(logServices)
    {
    }
    ...
    bool CTSE_SequenceTestConfig::PareseAssemblyConfig(const std::string& strConfigXml)
    {
        ...
        if (!strConfigXml.empty() && '<' == strConfigXml[0])
        {
            ...
            {
                mLogServices.ErrorLog("XML Buf Parese Fail\n");
                return false;
            }
        }
        else
        {
            ...
            {
                mLogServices.ErrorLogFormat(L"Config Not Exist(XML:%s)\n",JGW_A2W(strConfigXml).c_str());
                return false;
            }
        }
