...
namespace JGW
{
	...
	CTSE_GlobalEnvironment::CTSE_GlobalEnvironment()
	{
		grwMutex = new WR_Mutex();
	}

	CTSE_GlobalEnvironment::~CTSE_GlobalEnvironment()
	{
		delete (WR_Mutex*)grwMutex;
	}
    ...
    {
        readLock lockWrite(*(WR_Mutex*)grwMutex); 
        ...
        return it->second;
#else
        return GetIPCGlobalVariable()->GetGlobalVariableValue(itemName);
#endif     
    }
    ...
    {
        writeLock lockWrite(*(WR_Mutex*)grwMutex); 
#ifndef USE_IPC_GLOBALVARIABLE
        mmapKeyValue[itemName] = itemValue;
#else
        GetIPCGlobalVariable()->PutGlobalVariableNameValue(itemName,itemValue);
#endif
    }
    ...
    void CTSE_GlobalEnvironment::PutAnsiCopyDataMessageQueue(const std::string& strMessage)
    {
        if (!strMessage.empty())
        {
            writeLock lockWrite(*(WR_Mutex*)grwMutex);
    ...
    std::string CTSE_GlobalEnvironment::GetAnsiCopyDataMessageQueue()
    {
