...
namespace JGW
{
    ...
    CCJGW_DevicePoolThread::CCJGW_DevicePoolThread(void):mhMainHwnd(NULL)
    {
        ...
        g_nUserPortSort = GetPrivateProfileInt(_T("MAIN" ),_T( "DETECT_INST_PORT_NO"),0,JGW_GetUseFilterPortConfigFilePath() );
    }
    ...
    bool CCJGW_DevicePoolThread::StartDevicePoolThread(HWND hWnd,const wchar_t* strConfigPath)
    {
        ...
        return (usbConfig.LoadUsbConfigToConfigPath(strConfigPath,mmapsGuidUsbConfig) 
            && mSortUsbInfoConfig.LoadUsbFilterPortConfig(JGW_GetUseFilterPortConfigFilePath())
            && CreateMessageThread(0,0,true));
    }
    ...
    void CCJGW_DevicePoolThread::OnRspDeviceStatusChangeMsg(WPARAM wParam,LPARAM lParam)
    {
        ...
        //! 过滤掉未注册的GUID信息
        if (mmapGuidDevPoolInfo.end() == mmapGuidDevPoolInfo.find(lpDeviceMsg->m_guid)) 
        {
            lpDeviceMsg->Dec();
            return ;
        }
        ...
        if (mSortUsbInfoConfig.IsFilterUsbPidVid(lpDeviceMsg->m_strDbcc_name.c_str()))
        {
            lpDeviceMsg->Dec();
            return ;
        }
    ...
    void CCJGW_DevicePoolThread::DispatchDeviceMsg(SDeviceMsg_Info* lpdevice)
    {
        ...
        if (mmapCacheDeviceInfo.end() == mmapCacheDeviceInfo.find(lpdevice->m_strDbcc_name.c_str()))
        {
            ...
            if (E_ADB_GUID_TYPE == psGuidUsbConfig->meGuidType )
                lpdevice->m_strSeralNumber = mUsbEnumerator.GetSerialNumber();
            //! cache dbcc name
    ...
    {
        int nSortIndex = 0;
        std::wstring strPhysicalAddress;

        for ( int i = 0;i < 3;i ++)
        {
            //获取端口索引
            strPhysicalAddress = mUsbEnumerator.GetPhysicalAddress(strInstanceID);
            if (!strPhysicalAddress.empty())
            {
                nSortIndex = mSortUsbInfoConfig.GetUsbSortIndexToPhysicalAddress(strPhysicalAddress.c_str());
                DEVICEPOOL_LOG_INFO_FORMAT(L"[DevicePool] GetDownModeUsbSortIndex 正在注册[%d] %s",nSortIndex,strInstanceID);
                return nSortIndex;
            }      
        }
        return -1;
