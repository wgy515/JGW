...
namespace JGW
{
    CCJGW_USBEnumerator::CCJGW_USBEnumerator(void)/*:mvFilteUsbPidVid(NULL)*/
    {
    ...
    std::wstring CCJGW_USBEnumerator::GetPhysicalAddress(const std::wstring& strDbccName)
    {
        ...
        if (INVALID_HANDLE_VALUE == hDevInfo.get()) 
		{
			DEVICEPOOL_LOG_ERROR(L"GetPhysicalAddress::SetupDiGetClassDevs INVALID_HANDLE_VALUE");
			return L"";
		}

        SP_DEVINFO_DATA spDevInfoData = {0};
        spDevInfoData.cbSize = sizeof(SP_DEVINFO_DATA);
        if (!SetupDiEnumDeviceInfo(hDevInfo.get(), 0,&spDevInfoData)) 
		{
			DEVICEPOOL_LOG_ERROR(L"GetPhysicalAddress::SetupDiEnumDeviceInfo Fail");
			return L"";
		}
    ...
    std::wstring CCJGW_USBEnumerator::GetPhysicalAddressToHDEVINFO(HDEVINFO hDevInfo,PSP_DEVINFO_DATA pSpDevInfoData)
    {
        ...
        if (CR_SUCCESS != CM_Get_Parent(&pParentDevInst,pSpDevInfoData->DevInst,0) || INVALID_DEVINST_VALUE == pParentDevInst)
		{
			DEVICEPOOL_LOG_ERROR(L"GetPhysicalAddressToHDEVINFO::CM_Get_Parent Fail 1");
			return L"";
		}
        while (!crResult)
        {
            ...
            if (NULL != _tcsstr((const wchar_t*)mszDevInfoBuf,L"IUSB3"))
            {
                ...
                if (!connectIndex) 
				{
					DEVICEPOOL_LOG_ERROR_FORMAT(L"GetPhysicalAddressToHDEVINFO::GetConnectionIndex FaiL(%s)",ParentPNPDevicePath.c_str());
					return L"";
				}
		...
		DEVICEPOOL_LOG_ERROR(L"GetPhysicalAddressToHDEVINFO::CM_Get_Parent Fail 2");
        return L"";
    }
