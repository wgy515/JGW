...
#define PUMA_USB_SERVIER "usbser"

#define PUMA_VID "VID_2932"
...
#define PUMA_MAX_BUF_TX_RX 8 * 1024 * 1024
//! device_size 0x100000 8 Mbit
#define PERASO_DEVICE_SIZE 0x100000
...
#define PRINT_ERRORCODE(ec,msg)\
    if (ec) dbg(LOG_ERROR,msDownloadProgressInfo.mnIndex,msg,JGW_A2W(ec.message()).c_str());
...
namespace JGW
{
    ...
    const char eraseChipCmd[] = "erase a f8000000";
...
#define RESULT_OK(result) (result < ((1 << eDownloadResult_maxWarning) << 1))
    ...
    enum EDownloadStep
    {
        ...
        E_SPI_SetBaud,
        E_FLASH_Identity,
        E_CHECK_FLASH_BLANK, //! 删除flash 信息
        E_ERASE_FLASH,
    ...
    typedef struct  
    {
        ...
        bool mbFormatPDBImage; //! 是否格式化PDB镜像
        ...
        const std::vector<S_PUMA_IMAGE_DATA_INFO>* mvPumaEraseImageDataInfo;
    ...
    struct SMultiunitDownloadFWParam
    {
        SMultiunitDownloadFWParam():m_bAutoSwitchComport(false),mbValidateImage(true){}
        bool m_bAutoSwitchComport;
        bool mbValidateImage;
        bool mbFormatPDBImage;
        std::string mstrImageFilePath;
    };
}
