...
#define MULTIPLE_TEST_CONFIG L"MultipleTest.ini"
...
namespace JGW
{
    ...
    */
    ...
    typedef struct  
    {
        ...
        //! 下载时间
        size_t mnTestTimeSec;
        //! 下载出现超时时间
        size_t mnTestTimeOutSec;
        //！保存测试log目录
        ...
        //! 下载或者升级配置文件格式如下 xx=xx,aa=aa,bb=bb,cc=cc
        ...
        std::wstring mstrConfigTSEParam;
        //! TSE 原始参数
        //std::wstring mstrSourceTSEParam;
        //! 已经格式化TSE全局环境变量参数 => TSE_PARAM=XX TSE_PARAM1=XX等相关格式
        std::wstring mstrFormatTSEParam;
        //! TSE 原始参数 对应的参数常量
        //std::map<E_GSTSE_ENVNAME,std::wstring> mmapeSourceTSEParam;
        std::map<std::wstring,std::wstring> mmapTSEParamValues;
        //! 网卡设备相关配置
        S_NETCARD_CONFIG msNetCardConfig;
        //! 设备排序方案配置信息
        S_DEVICEQUEUE_PROGRAM_INFO msDeviceProgramInfo;
